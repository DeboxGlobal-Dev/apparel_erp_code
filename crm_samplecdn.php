      ...
      <div class="row">
        <div class="col-xl-12">
          <div class="card-header header-elements-inline bg-blue-700 filterable" style="padding: 10px;">
            ...
            <a href="showpage.crm?module=samplecdn&add=yes" class="btn bg-teal-400 addnotify" aria-expanded="false" style="    background-color: #03d873b8;"><i class="fa fa-plus" aria-hidden="true"></i> Create New</a> </div>
          <div class="card">
...
$stylerefCondition = 'and id="'.decode($_GET['stylerefid']).'"';
 ...
 $where='where cdntype=2 order by id desc';
...
$rs=GetRecordList($select,'cdnMaster',$where,$limit,$page,$targetpage);
...
?>
                      <tr>
                        <td><a href="showpage.crm?module=<?php echo $_GET['module']; ?>&add=yes&brandid=<?php echo encode($resultlists['brandId']); ?>">
                          <?php

                          $rrrr=GetPageRecord('*','brandMaster','1 and id="'.$resultlists['brandId'].'" ');

                         $brand=mysqli_fetch_array($rrrr);

                             ?>
                          <?php echo $brand['name']; ?> 1</a></td>
                        <td><?php echo makeQueryId($resultlists['id']); ?></td>
                        <td><?php echo $resultlists['cdndate'] ?></td>
                        <td><?php echo $resultlists['factorylocation'] ?></td>
                        <td><?php

                          $rrrr=GetPageRecord('*','buyerMaster','1 and id="'.$resultlists['buyerId'].'" ');

                         $buyer=mysqli_fetch_array($rrrr);

                             ?>
                          <?php echo $buyer['name']; ?></td>
                      </tr>
                      <?php } ?>
                    </tbody>
                  </table>
