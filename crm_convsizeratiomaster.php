			...
			<div class="col-xl-12">
				<div class="card-header header-elements-inline bg-info-700">
					<div class="col-xl-9"><h5 class="card-title"><?php echo $pageName; ?></h5></div>
					 ...
					 <a href="#" onclick="opmodalpop(' Add Size Ratio','modalpop.php?action=<?php echo $_GET['module']; ?>','600px','auto');" data-toggle="modal" data-target="#modalpop" class="btn bg-teal-400" aria-expanded="false" style="background-color: #03d873b8;"><i class="fa fa-plus" aria-hidden="true"></i> Create New</a>
					 </div></div>
					</div>
					<div class="card">
					...
					<table class="table table-bordered table-hover datatable-highlight dataTable no-footer" id="DataTables_Table_2" role="grid" aria-describedby="DataTables_Table_2_info">
						<thead>
							<tr role="row">
							<th class="sorting_asc" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="ascending">Destination</th>
							...
							<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1">Sub&nbsp;Category</th>
														<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1">Size&nbsp;Range</th>

							<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1">Size&nbsp;Ratio</th>
						...
						?>
							<tr role="row" class="odd">
								<td tabindex="0" class="sorting_1"> <a href="#" onclick="opmodalpop(' Edit Size Ratio','modalpop.php?action=<?php echo $_GET['module']; ?>&id=<?php echo encode($resultlists['id']); ?>','600px','auto');" data-toggle="modal" data-target="#modalpop"><?php echo $resultlists['destination']; ?></a></td>

								<td><?php
							$rs=GetPageRecord("*",'buyerMaster','1 and id="'.$resultlists['buyerId'].'"');
							$buyer = mysqli_fetch_array($rs);
							echo $buyer['name'];?>
						        </td>
								<td>
									<?php
							$rs=GetPageRecord("*",'brandMaster','1 and id="'.$resultlists['brandId'].'"');
							$brand = mysqli_fetch_array($rs);
							echo $brand['name'];?>
								</td>

								<td>
									<?php
						$rs=GetPageRecord("*",'subCategoryMaster','1 and id="'.$resultlists['subCategoryId'].'"');
							$category = mysqli_fetch_array($rs);
							echo $category['name'];?>
								</td>
																<td>
																<?php
						$rs=GetPageRecord("*",'sizerangeMaster','1 and id="'.$resultlists['sizeRangeId'].'"');
							$range = mysqli_fetch_array($rs);
							echo $range['name'];?>
																</td>

																<td>
																	<?php echo $resultlists['sizeRatio'] ?>
																</td>
