					...
					<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer"> <div class="datatable-scroll"><table class="table table-bordered table-hover datatable-highlight dataTable no-footer" id="DataTables_Table_2" role="grid" aria-describedby="DataTables_Table_2_info">
						<thead>
							<tr role="row">
							  ...
							  <th class="sorting" tabindex="0" aria-controls="DataTables_Table_2" rowspan="1" aria-label="Last Name: activate to sort column ascending">Type</th>
							  ...
							  <th class="sorting" tabindex="0" aria-controls="DataTables_Table_2" rowspan="1" colspan="1" aria-label="DOB: activate to sort column ascending">Phone</th>
...
$where='where  '.$wheresearchassign.' '.$wheresearch.' and name!="" '.$assignto.' and deletestatus=0 order by dateAdded desc';
$page=$_GET['page'];
...
?>
						...
						<tr role="row" class="odd">
								<td class="sorting_1"><a class="shorttextcard"  title="<?php echo $resultlists['name']; ?>" style=" color:#2196f3; display:block; min-width:120px; max-width:120px;" href="#" onclick="view('<?php echo encode($resultlists['id']); ?>');" ><?php echo strip($resultlists['name']); ?></a></td>
								<td class=""><?php if($resultlists['bussinessType']==1) {  echo ' Corporate'; } else { echo 'Agent'; } ?></td>
								...
								<td class="text-left"><div class="shorttextcard" title="<?php echo showmonthnamedate($resultlists['dateAdded']);?>"><?php echo showmonthnamedate($resultlists['dateAdded']);?></div></td>
							</tr>
        ...
        if (pie_donut_element) {
            ...
            pie_donut.setOption({
                ...
                series: [{
					...
					$rs='';
					$select='*';
					$where=' deletestatus=0 and bussinessType=1 or bussinessType=2 group by bussinessType order by id asc';
					$rs=GetPageRecord('COUNT(id) as totalid, bussinessType ',_CORPORATE_MASTER_,$where);
					while($resListing=mysqli_fetch_array($rs)){ ?>
						{value: <?php echo $resListing['totalid']; ?>, name: '<?php if($resListing['bussinessType']=='1'){ echo 'Corporate'; } if($resListing['bussinessType']=='2'){ echo 'Agent'; }  ?>'},
					<?php } ?>
                    ]
                }]
            });
        ...
        if (pie_basic_element) {
            ...
            pie_basic.setOption({
                ...
                series: [{
					...
					$rs=GetPageRecord('COUNT(id) as totalid, cityId ',_CORPORATE_MASTER_,$where);
