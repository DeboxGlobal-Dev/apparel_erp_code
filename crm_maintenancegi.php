        ...
        <div class="col-xl-12">
          <div class="card-header header-elements-inline bg-blue-700 filterable" style="padding: 10px;">
            ...
            <div class="col-xl-3" style="padding-right: 0px;"><div class="btn-group justify-content-center" style="float:right;">
                                <a href="showpage.crm?module=maintenancegi&add=yes" class="btn bg-teal-400 addnotify" aria-expanded="false" style="    background-color: #03d873b8;"><i class="fa fa-plus" aria-hidden="true"></i> Create New</a>
                              </div> </div>
          </div>
          <div class="card">
...
$rs=GetRecordList($select,'maintenancegi_Master','where status=1 order by id desc',$limit,$page,$targetpage);
$totalentry=$rs[1];
$paging=$rs[2];
...
<tr role="row" class="odd">
	<td><a href="showpage.crm?module=maintenancegi&add=yes&id=<?php echo encode($resultlists['id']); ?>">
    <?php echo $resultlists['requisitionno']; ?></a></td>
 ...
 <td><?php echo date('d-m-Y',$resultlists['dateAdded']); ?></td>
   <?php
                  ...
                  $fcref=GetPageRecord('*','userMaster','1 and id="'.$resultlists['requestedby'].'"') ;
                              $refData=mysqli_fetch_array($fcref);
                              ?>
   <td><?php echo $refData['firstName']; ?>&nbsp;<?php echo $refData['lastName']; ?></td>
   <?php

                  $fcrefdx=GetPageRecord('*','departmentMaster','1 and  id="'.$resultlists['requesteddepartment'].'"') ;
                              $refDatadx=mysqli_fetch_array($fcrefdx);
   ?>
   <td><?php echo $refDatadx['name']; ?></td>
   <td>
       <?php
       if($resultlists['approvedstatus']=='1' ){
           ?>

           <span class="badge" style="background-color:green; color:#fff; position: relative;width: 142px; font-size: 11px; padding: 6px;">Approved</span>
      <?php }else{
          ?>

                     <span class="badge" style="background-color:green; color:#fff; position: relative;width: 142px; font-size: 11px; padding: 6px;">Unapproved</span>
<?php
      }
      ?>
    ...
    <td>	<a href="tcpdf/examples/generatemaint.php?pageurl=<?php echo $fullurl; ?>download-maintenancegipdf.php?id=<?php echo encode($resultlists['id']); ?>" target="_blank" style="background: #e71c22; outline: none; color: #fff; padding: 5px; border-radius: 2px; cursor: pointer; width: 95px; display: block;">
