				...
				<div class="card-header header-elements-inline bg-info-700">
					<div class="col-xl-9"><h5 class="card-title"><?php echo $pageName; ?></h5></div>
					  <div class="col-xl-3" style="    padding-right: 0px;"><div class="btn-group justify-content-center" style="float:right;">
							 ...
							 <a href="#" onclick="opmodalpop(' Add Sub Category','modalpop.php?action=<?php echo $_GET['module']; ?>','600px','auto');" data-toggle="modal" data-target="#modalpop" class="btn bg-teal-400" aria-expanded="false" style="background-color: #03d873b8;"><i class="fa fa-plus" aria-hidden="true"></i> Create New</a>
					...
					<table class="table table-bordered table-hover datatable-highlight dataTable no-footer" id="DataTables_Table_2" role="grid" aria-describedby="DataTables_Table_2_info">
						<thead>
							<tr role="row">
							<th class="sorting_asc" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="ascending">Sub&nbsp;Category&nbsp;Name</th>
							<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1">Category&nbsp;Name</th>
						    <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1">Material</th>
							<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1">Gender</th>
						...
						$rs=GetRecordList($select,_SUB_CATEGORY_MASTER_,$where,$limit,$page,$targetpage);
						$totalentry=$rs[1];
						$paging=$rs[2];
						...
						?>
							<tr role="row" class="odd">
								<td tabindex="0" class="sorting_1"><a href="#" onclick="opmodalpop(' Edit Sub Category','modalpop.php?action=<?php echo $_GET['module']; ?>&id=<?php echo encode($resultlists['id']); ?>','600px','auto');" data-toggle="modal" data-target="#modalpop"><?php echo $resultlists['name']; ?></a></td>

								<td tabindex="0" class="sorting_1"><?php echo getCategoryName($resultlists['categoryId']); ?></td>


								<td>

								<?php
								$array =  explode(',', $resultlists['materialid']);
								foreach ($array as $item) {

								$where22='';
								$rs22='';
								$select22='name';
								$where22='id="'.$item.'" order by name asc';
								...
								<td>
								<?php
								if($resultlists['gender']==1){
								echo "Male";
								}
								if($resultlists['gender']==2){
								echo "Woman";
								}
								if($resultlists['gender']==3){
								echo "Kids";
								}
								...
								<td tabindex="0" class="sorting_1"><?php echo $resultlists['description']; ?></td>
								<td>
								<?php $select2='';
								$where2='';
								$rs2='';
