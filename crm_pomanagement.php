					...
					<table class="table table-bordered table-hover datatable-highlight dataTable no-footer" id="DataTables_Table_2" role="grid" aria-describedby="DataTables_Table_2_info">
						<thead style="background-color: #f5f5f5;">
							<tr role="row">
								...
								<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1">Confirmed&nbsp;Quantity</th>
								<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1">PO&nbsp;Qnty.&nbsp;Received</th>
...
?>
							...
							<td align="center"><?php echo $resultlists['displayId']; ?></td>
								...
								<td><a href="showpage.crm?module=poManage&add=yes&styleid=<?php echo encode($resultlists['id']); ?>"><?php echo '#'.$resultlists['styleRefId']; ?><?php if(countQueryunreadMails($resultlists['id'])!=0){ ?><div class="numberbubbol"><?php echo countQueryunreadMails($resultlists['id']); ?></div><?php } ?></a></td>
								...
								?>
							   <td align="center"><?php echo $resultqty['qtyTotal']; ?></td>
								<td>
									<?php
									$total=0;
				...
				while($operationData=mysqli_fetch_array($rrrr)){
                     $total+= $operationData['poQty'];
						...
						<a href="download-po.php?styleid=<?php echo encode($resultlists['id']); ?>" target="_blank" style="background: #0288d1; outline: none; color: #fff; padding: 5px; border-radius: 2px; cursor: pointer; width: 95px; display: block; float:left;  text-align:center;"><i class="fa fa-download" aria-hidden="true"></i> Excel</a>
