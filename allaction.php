...
parent.window.location.href =
    'page.de?section=accounts&view=yes&id=<?php echo $_POST['editId']; ?>&alt=<?php if($_REQUEST['editedityes']=='1'){ echo '2'; } else { echo '1'; } ?>';
</script>
<?php
...
<script>
parent.window.location.href =
    'page.de?section=contacts&view=yes&id=<?php echo $_POST['editId']; ?>&alt=<?php if($_REQUEST['editedityes']=='1'){ echo '2'; } else { echo '1'; } ?>';
</script>
<?php
...
<script>
parent.window.location.href =
    'page.de?section=supplier&view=yes&id=<?php echo $_POST['editId']; ?>&alt=<?php if($_REQUEST['editedityes']=='1'){ echo '2'; } else { echo '1'; } ?>';
</script>
<?php
...
<script>
parent.setupbox(
    'showpage.crm?module=brandmaster&view=yes&id=<?php echo encode($brandId); ?>&buyerId=<?php echo encode($buyerId); ?>'
    );
</script>
<?php }
...
$assignTo=clean($_SESSION['userid']);
...
$namevalue ='subject="'.$subject.'",buyerId="'.$buyerId.'",styleRefId="'.$styleRefId.'",techpackattachment="'.$file_name.'",colorbreakup="'.$colorbreakup.'",buyerStyleRefNo="'.$buyerStyleRefNo.'",receivedDate="'.$receivedDate.'",categoryId="'.$categoryId.'",subCategoryId="'.$subCategoryId.'",seasonId="'.$seasonId.'",divisionId="'.$divisionId.'",departmentId="'.$departmentId.'",displayId="'.$displayId.'",mailId="'.$_REQUEST['mailId'].'",attachmentFile="'.$file_name.'",assignTo="'.$assignTo.'",orderQty="'.$orderQty.'",receivedDate="'.$receivedDate.'",addedBy="'.$_SESSION['userid'].'",dateAdded="'.$dateAdded.'",stylestatus="1",defaultcostsheetVersionId="1",queryPriority="'.$queryPriority.'",styleType="'.$styleType.'",styleTypeId="'.$styleTypeId.'",remark="'.$remark.'",refrenceBy="'.$refrenceBy.'",attachmentNewMail="'.$attachmentNewMail.'",smv="'.$smv.'",efficiency="'.$efficiency.'",gender="'.$gender.'",brandId="'.$brandId.'",segment="'.$segment.'",merchantStyleNo="'.$merchantStyleNo.'",costingQty="'.$costingQty.'",projecQty="'.$projecQty.'",sizerange="'.$sizerange.'",sizeratio="'.$sizeratio.'",ocdDate="'.$ocdDate.'",pcdDate="'.$pcdDate.'",shipDate="'.$shipDate.'",sampleStyle="'.$sampleStlye.'",sampleFor="'.$sampleFor.'",productionStage="'.$productionStage.'",sampleType="'.$sampleType.'",parentStyleId="'.$parentStyleId.'",requestedDate="'.$requestedDate.'",expectedDate="'.$expectedDate.'",dispatchDate="'.$dispatchDate.'",dispatchDetail="'.$dispatchDetail.'",requestedBy="'.$requestedBy.'",riskPriority="'.$risk.'",repeatOrder="'.$repeatorder.'",merchant="'.$merchantname.'", styleColor="'.$styleColor.'",sample_indent="'.$sample_indent.'",masterStyleNo="'.$masterStyleNo.'"';
...
$valueEdition = implode(",",$_POST['valueEdition'.$valuesAddrs]);
...
$notes="<strong>Style created</strong> by <span class='assignClass'> ".getUserName($_SESSION['userid'])." </span>";

$styleassignment = 'styleId="'.decode($_POST['editId']).'",statusId=13,notes="'.$notes.'",assignTo="'.$_SESSION['userid'].'",dateAdded="'.$dateAdded.'"';
...
$valueEdition = implode(",",$_POST['valueEdition'.$valuescolor]);
...
$versionId = decode($_POST['versionId']);
	...
	if($checkCode == 'yes'){
		...
		$namevalue121 ='materialMasterId="'.$duplicateRecords['materialMasterId'].'",name="'.$duplicateRecords['name'].'",materialid="'.$duplicateRecords['materialid'].'",materialType="'.$duplicateRecords['materialType'].'",subCategoryId="'.$duplicateRecords['subCategoryId'].'",status="'.$duplicateRecords['status'].'",styleId="'.$editId.'",sr="'.$duplicateRecords['sr'].'",assignTo="'.$duplicateRecords['assignTo'].'",costsheetVersionId="'.$versionIdNew.'",qtyStatus="'.$duplicateRecords['qtyStatus'].'",priceStatus="'.$duplicateRecords['priceStatus'].'",vendorStatus="'.$duplicateRecords['vendorStatus'].'",materialdescriptionid="'.$duplicateRecords['materialdescriptionid'].'",assignToPurMerchant="'.$duplicateRecords['assignToPurMerchant'].'",colorSeparate="'.$duplicateRecords['colorSeparate'].'",sizeSeparate="'.$duplicateRecords['sizeSeparate'].'",addMaterialFrom="'.$duplicateRecords['addMaterialFrom'].'"';

		$addddddd = addlistinggetlastid('styleSubCategoryMaster',$namevalue121);

		$whered12='styleId="'.$editId.'" and costsheetVersionId="'.$versionId.'" and stylesubtabid="'.$duplicateRecords['id'].'"';
		$rsd12=GetPageRecord('*','techPackDetailMaster',$whered12);
		$duplicateRecords12=mysqli_fetch_array($rsd12);


		$namevaluetpDetail='costsheetVersionId="'.$versionIdNew.'",sectionType="'.$duplicateRecords12['sectionType'].'",styleId="'.$duplicateRecords12['styleId'].'",typeOfMachine="'.$duplicateRecords12['typeOfMachine'].'",remark="'.$duplicateRecords12['remark'].'",bomCategoryId="'.$duplicateRecords12['bomCategoryId'].'",bomSubCategoryId="'.$duplicateRecords12['bomSubCategoryId'].'",bomSerialNo="'.$duplicateRecords12['bomSerialNo'].'",bomQuality="'.$duplicateRecords12['bomQuality'].'",bomColorFirst="'.$duplicateRecords12['bomColorFirst'].'",bomColorSecond="'.$duplicateRecords12['bomColorSecond'].'",bomPlacement="'.$duplicateRecords12['bomPlacement'].'",bomAvg="'.$duplicateRecords12['bomAvg'].'",bomWidth="'.$duplicateRecords12['bomWidth'].'",bomWidthUnit="'.$duplicateRecords12['bomWidthUnit'].'",bomUnit="'.$duplicateRecords12['bomUnit'].'",bomSupplier="'.$duplicateRecords12['bomSupplier'].'",bomExInhouseDate="'.$duplicateRecords12['bomExInhouseDate'].'",bomComment="'.$duplicateRecords12['bomComment'].'",bomStatus="'.$duplicateRecords12['bomStatus'].'",cid="'.$duplicateRecords12['cid'].'",bomUSD="'.$duplicateRecords12['bomUSD'].'",bomINR="'.$duplicateRecords12['bomINR'].'",bomRate="'.$duplicateRecords12['bomRate'].'",bomvalueonepc="'.$duplicateRecords12['bomvalueonepc'].'",wastagePersent="'.$duplicateRecords12['wastagePersent'].'",avgIncWastage="'.$duplicateRecords12['avgIncWastage'].'",storesupplier="'.$duplicateRecords12['storesupplier'].'",addToCost="'.$duplicateRecords12['addToCost'].'",stylesubtabid="'.$addddddd.'",matPrice="'.$duplicateRecords12['matPrice'].'",matCurrency="'.$duplicateRecords12['matCurrency'].'"';
...
<script>
parent.setupbox(
    'showpage.crm?module=<?php echo $_POST['module']; ?>&alt=1&stylerefid=<?php echo $_REQUEST['stylerefid']; ?>');
</script>
<?php }
...
<script>
parent.setupbox(
    'showpage.crm?module=<?php echo $_POST['module']; ?>&alt=2&stylerefid=<?php echo $_REQUEST['stylerefid']; ?>');
</script>
<?php }
...
$sapCode=addslashes($_POST['sapCode']);

$materialStatus=clean($_POST['materialStatus']);

$hsnCodeId=addslashes($_POST['hsnCodeId']);
$widthId = implode(",",$_POST['widthId']);
...
$namevalue ='name="'.$name.'",materialtype="'.$categoryId.'",description="'.$description.'",materialimage="'.$file_name.'",status="'.$status.'",dateAdded="'.$dateAdded.'",addedBy="'.$_SESSION['userid'].'",materialUniqueId="'.$randId.'",materialStatus="'.$materialStatus.'",materialSubTypeId="'.$materialSubTypeId.'",finishId="'.$finishId.'",hsnCodeId="'.$hsnCodeId.'",gsm="'.$gsm.'",widthId="'.$widthId.'"';
...
$hsnCodeId=stripslashes($_POST['hsnCodeId']);
$materialStatus=clean($_POST['materialStatus']);
$widthId = implode(",",$_POST['widthId']);
...
$namevalue ='name="'.$name.'",materialtype="'.$categoryId.'",description="'.$description.'",materialimage="'.$file_name.'",status="'.$status.'",addedBy="'.$_SESSION['userid'].'",modifyDate="'.$modifyDate.'",modifyBy="'.$_SESSION['userid'].'",materialUniqueId="'.$randId.'",materialStatus="'.$materialStatus.'",materialSubTypeId="'.$materialSubTypeId.'",finishId="'.$finishId.'",hsnCodeId="'.$hsnCodeId.'",gsm="'.$gsm.'",widthId="'.$widthId.'"';
...
$namevalue ='name="'.$name.'",buyerId="'.$buyerId.'",groupId="2",accountId="'.$accountId.'",buyerShortName="'.$shortname.'",buyeremail="'.$bemail.'",buyerphone="'.$bphone.'",status="'.$status.'",addedBy="'.$_SESSION['userid'].'",modifyDate="'.$modifyDate.'"';
...
$namevalue ='supplierId="'.$supplierid.'",name="'.$suppliername.'",groupId="1",accountId="'.$accountId.'",email="'.$email.'",phone="'.$phone.'",addedBy="'.$_SESSION['userid'].'",dateAdded="'.$dateAdded.'",modifyBy="'.$_SESSION['userid'].'",modifyDate="'.$dateAdded.'"';
...
if(trim($_POST['action'])=='assignStyle' && $_POST['styleId']!='' && $_POST['assignTo']!=''){
...
//Update the previous stage mark done
$rsCheck=GetPageRecord('profileId','userMaster','id="'.$_SESSION['userid'].'"');
$resListingCheck=mysqli_fetch_array($rsCheck);

$where1='styleId="'.$styleId.'" and assignTo="'.$_SESSION['userid'].'" and profileId="'.$resListingCheck['profileId'].'"';
$namevalue1 ='stageStatus="1"';
$update1 = updatelisting('styleAssignmentMaster',$namevalue1,$where1);

//assign to the next person
$namevalue ='styleId="'.$styleId.'",assignTo="'.$assignTo.'",notes="'.$notes.'",statusId=5,dateAdded="'.$dateAdded.'",
addedBy="'.$_SESSION['userid'].'",profileId="'.$profileId.'"';

$adds = addlisting('styleAssignmentMaster',$namevalue);

$_SESSION['MSG'] = 'Successfully Assigned';
...
<script>
var abc = parent.document.getElementById('messagevalcount<?php echo $bomSerialNo; ?><?php echo $costversionid; ?>')
    .innerText;

var finalcommetcount = Number(abc) + 1;

parent.document.getElementById('messagevalcount<?php echo $bomSerialNo; ?><?php echo $costversionid; ?>').innerHTML =
    finalcommetcount;
</script>
<?php } ?>
...
parent.document.getElementById('qtyStatus<?php echo $bomSerialNo; ?><?php echo $costversionid; ?>').innerHTML =
    finalcommetcount;
...
<script>
var abc = parent.document.getElementById('priceStatus<?php echo $bomSerialNo; ?><?php echo $costversionid; ?>')
    .innerText;
...
parent.document.getElementById('priceStatus<?php echo $bomSerialNo; ?><?php echo $costversionid; ?>').innerHTML =
    finalcommetcount;
...
<script>
var abc = parent.document.getElementById('vendorStatus<?php echo $bomSerialNo; ?><?php echo $costversionid; ?>')
    .innerText;
...
parent.document.getElementById('vendorStatus<?php echo $bomSerialNo; ?><?php echo $costversionid; ?>').innerHTML =
    finalcommetcount;
...
<?php  if($_REQUEST['commnetType']=='0'){ ?>
<script>
parent.opmodalpop('<?php echo $heading; ?>',
    'modalpop.php?action=materialcostchat&styleId=<?php echo $_POST['editId']; ?>&srno=<?php echo $bomSerialNo; ?>&materialType=<?php echo $materialType; ?>&materialId=<?php echo $materialId; ?>&costversionid=<?php echo $costversionid; ?>&commnetType=<?php echo $_REQUEST['commnetType']; ?>&n=<?php echo $adds; ?>&id=<?php echo $id; ?>&fortTimeSlot=<?php echo $fortTimeSlot; ?>',
    '600px', 'auto');
</script>
<?php } else { ?>
<script>
...
<script>
parent.setupbox(
    'showpage.crm?module=comparesuppliercost&view=yes&styleid=<?php echo encode($styleId); ?>&costsheetVersionId=<?php echo $costsheetVersionId;  ?>'
    );
</script>
<?php
...
<script>
parent.setupbox(
    'showpage.crm?module=comparevendorcost&view=yes&styleid=<?php echo encode($styleId); ?>&pdoutsource=yes');
</script>
<?php
...
<script>
parent.setupbox(
    'showpage.crm?module=<?php echo $_POST['module']; ?>&view=yes&id=<?php echo encode($parentId); ?>&alt=1');
</script>
<?php }
...
<script>
parent.setupbox(
    'showpage.crm?module=<?php echo $_POST['module']; ?>&view=yes&id=<?php echo decode($_POST['parentId']); ?>&alt=2'
    );
</script>
...
if(trim($_POST['action'])=='editgrn' && trim($_POST['editId'])!='' && trim($_POST['supplierId'])!=''){
...
$krsk=GetPageRecord('voucherFlag','grnMaster','1 and id="'.$editId.'" ');
$result=mysqli_fetch_array($krsk);
$vFlag = $result['voucherFlag'];

if($vFlag == 0){

$krsk1=GetPageRecord('accountId','suppliersMaster','1 and id="'.$supplierId.'" ');
$result1=mysqli_fetch_array($krsk1);
$accountId = $result1['accountId'];

$jsonData = '{
  "VoucherDate": "'.date("Y-m-d").'",
  "Narration": "Add by prasang",
  "Type": "Purchase",
...
$url = $fullurl."accounts/addVoucherEntryAPI.php";
$response = postCurlData($url,$jsonData);
$res = json_decode($response,true);

}

if($res[Status] == 0 || $vFlag == 1) {
$where = 'id="'.$editId.'"';

$namevalue ='grnNo="'.$grnNo.'",factoryId="'.$factoryId.'",supplierId="'.$supplierId.'",docNo="'.$docNo.'",docDate="'.$docDate.'",qcStatus="'.$qcStatus.'",eWayBill="'.$eWayBill.'",eWayBillDate="'.$eWayBillDate.'",ginNo="'.$ginNo.'",ginDate="'.$ginDate.'",eSungamNo="'.$eSungamNo.'",supplierPurchaseOrderId="'.$supplierPurchaseOrderId.'",chargesDetail="'.$chargesDetail.'",workPlaceId="'.$workPlaceId.'",gateEntryNo="'.$gateEntryNo.'",formNo="'.$formNo.'",transporter="'.$transporter.'",billitiNo="'.$billitiNo.'",eWay="'.$eWay.'",address="'.$address.'",stateCode="'.$stateCode.'",ieCode="'.$ieCode.'",hsn="'.$hsn.'",amount="'.$amount.'",cgst="'.$cgst.'",acceptBy="'.$acceptBy.'",voucherFlag="'.$voucherFlag.'",preparedBy="'.$preparedBy.'",prepareDate="'.$prepareDate.'"';
...
if($editId!=''){
  $where='id='.$editId.'';
  $update = updatelisting('sampleDispatchMaster',$namevalue,$where);
}else{
...
<script>
parent.setupbox(
    'showpage.crm?module=<?php echo $_POST['module']; ?>&add=yes&styleid=<?php echo $_POST['styleId']; ?>&alt=2');
</script>
<?php
...
$year = date("dmy");
///Order Seq fetch
//$rs=GetPageRecord('orderSeq','indentCreationMaster','1 and bomPoStatus=1 order by id desc');
//$rest = mysqli_fetch_array($rs);

$orderSeq = GetOrderSequenceNo();
$time = time();
$processName = $poType;

$uniqueOrderId1 = $processName.'ORDER'.$year.$orderSeq;

$uniquePonNumber = $uniqueOrderId1;

foreach($array as $itennss) {

	 $update = updatelisting('indentCreationMaster','bomPoStatus=1,poNumber="'.$uniquePonNumber.'",orderSeq="'.$orderSeq.'",generatedBy="'.$_SESSION['userid'].'"','id="'.$itennss.'"');
...
$Type = "Debit Voucher";
foreach($_POST['accountHeadId'] as $accountRow){

$transactionJson.= '{
             "AccountCode":"'.$_POST['accountHeadId'][$no].'",
...
if($_POST['credit'][$no] == 0){
$amount = $_POST['debit'][$no];
$type = 'D';
}
if($_POST['debit'][$no] == 0){
$amount = $_POST['credit'][$no];
$type = 'C';
}
...
$finalDisplayId = $type.'-000'.rand(10,1000);
...
$namevaluein ='invoiceNo="'.$finalDisplayId.'", status=2';
$update = updatelisting('packinglistMaster',$namevaluein,'id='.decode($_POST['pid']).'');

$namevalueinvs ='invoiceNumber="'.$finalDisplayId.'"';
$updateinvs = updatelisting('innvoiceMaster',$namevalueinvs,'id="'.decode($_POST['inid']).'"');
/////////////////////////Create Invoice End///////////////////////////

$invdata=GetPageRecord('*','innvoiceMaster','invoiceNumber="'.$finalDisplayId.'"');
$invdataset=mysqli_fetch_array($invdata);

/////////////////////////////POST DATA TO LEDGER START/////////////////////////////////////
if($invoiceType==2){
  $jsonPost = '{
    "StyleId":"'.$styleData['styleRefId'].'",
    "InvoiceNo":"'.$finalDisplayId.'",
    "VoucherDate":"'.$invdataset['gstdate'].'",
    "EntryDate":"'.date('Y-m-d').'",
    "UserId":"'.$_SESSION['userid'].'",
    "ip":"152.58.24.163",
    "BankAccount":"CP001",
    "ListOfTransaction":[{
        "AccountName":"'.$buyerData['buyerId'].'",
        "Debit":"'.$invdataset['netweight'].'",
        "Narration":"for style '.$styleData['styleRefId'].'"
    }]
  }';
}

$url = $serverurlapi."vouchers/invoice_api.php";
$resultData = postCurlData($url,$jsonPost);
logger("RESPONSE RETURN FROM INVOICE API: ".$resultData);

/////////////////////////////POST DATA TO LEDGER END/////////////////////////////////////
...
// parent.window.location.href =
//     '<?php echo $fullurl; ?>tcpdf/examples/generateinvoice.php?pageurl=<?php echo $fullurl; ?>invoice.php?s=<?php echo $_REQUEST['editId']; ?>';
</script>
<?php }
...
$price_kg=clean($_POST['price_kg']);

$curr_ist=clean($_POST['curr_ist']);

$mtr_kg=clean($_POST['mtr_kg']);

$yrd_kg=clean($_POST['yrd_kg']);

$count_th=clean($_POST['count_th']);

$count_four=clean($_POST['count_four']);

$count_five=clean($_POST['count_five']);

$wcount_th=clean($_POST['wcount_th']);

$wcount_four=clean($_POST['wcount_four']);

$wcount_five=clean($_POST['wcount_five']);
...
$yarnleadtimes_moq=clean($_POST['yarnleadtimes_moq']);

$yarnleadtimesprice_kg=clean($_POST['yarnleadtimesprice_kg']);

$yarnleadtimesprice_yd=clean($_POST['yarnleadtimesprice_yd']);

$yarnleadtimes_feeder=clean($_POST['yarnleadtimes_feeder']);

$yarnleadtimes_auto=clean($_POST['yarnleadtimes_auto']);

$weavingleadtime_moq=clean($_POST['weavingleadtime_moq']);

$weavingleadtimeprice_kg=clean($_POST['weavingleadtimeprice_kg']);

$weavingleadtimeprice_yd=clean($_POST['weavingleadtimeprice_yd']);

$weavingleadtime_feeder=clean($_POST['weavingleadtime_feeder']);

$weavingleadtime_auto=clean($_POST['weavingleadtime_auto']);

$printleadtime_moq=clean($_POST['printleadtime_moq']);
...
$printleadtimeprice_kg=clean($_POST['printleadtimeprice_kg']);

$printleadtimeyard_yd=clean($_POST['printleadtimeyard_yd']);

$printleadtimesolid=clean($_POST['printleadtimesolid']);

$printleadtimeprint=clean($_POST['printleadtimeprint']);



$printleadtimeyarn=clean($_POST['printleadtimeyarn']);

$printleadtimeheather=clean($_POST['printleadtimeheather']);

$printleadtime_feeder=clean($_POST['printleadtime_feeder']);



$printleadtime_auto=clean($_POST['printleadtime_auto']);

$le_leadtime_moq=clean($_POST['le_leadtime_moq']);

$le_leadtimeprice_kg=clean($_POST['le_leadtimeprice_kg']);

$le_leadtimeyard_yd=clean($_POST['le_leadtimeyard_yd']);

$le_leadtimesolid=clean($_POST['le_leadtimesolid']);

$le_leadtimeprint=clean($_POST['le_leadtimeprint']);

$le_leadtimeyarn=clean($_POST['le_leadtimeyarn']);
...
$le_leadtimeheather=clean($_POST['le_leadtimeheather']);

$le_leadtime_feeder=clean($_POST['le_leadtime_feeder']);

$le_leadtime_auto=clean($_POST['le_leadtime_auto']);

$dyeingleadtime_moq=clean($_POST['dyeingleadtime_moq']);

$dyeingleadtimeprice_kg=clean($_POST['dyeingleadtimeprice_kg']);

$dyeingleadtimeyard_yd=clean($_POST['dyeingleadtimeyard_yd']);

$dyeingleadtime_feeder=clean($_POST['dyeingleadtime_feeder']);

$dyeingleadtime_auto=clean($_POST['dyeingleadtime_auto']);

$testingleadtime_moq=clean($_POST['testingleadtime_moq']);
...
$testingleadtimeprice_kg=clean($_POST['testingleadtimeprice_kg']);

$testingleadtimeyard_yd=clean($_POST['testingleadtimeyard_yd']);

$testingleadtime_feeder=clean($_POST['testingleadtime_feeder']);

$testingleadtime_auto=clean($_POST['testingleadtime_auto']);



$totalleadtime_moq=clean($_POST['totalleadtime_moq']);

$totalleadtimeprice_kg=clean($_POST['totalleadtimeprice_kg']);

$totalleadtimeyard_yd=clean($_POST['totalleadtimeyard_yd']);

$totalleadtime_feeder=clean($_POST['totalleadtime_feeder']);

$totalleadtime_auto=clean($_POST['totalleadtime_auto']);
...
$sizeRangeName=clean($_POST['sizename']);

$description=clean($_POST['description']);

$dateAdded=time();

$namevalue ='name="'.$size.'",sizeRangeId="'.$sizeRangeId.'",sizeRangeName="'.$sizeRangeName.'",description="'.$description.'",dateAdded="'.$dateAdded.'",addedBy="'.$_SESSION['userid'].'"';
...
$namevalue ='name="'.$size.'",sizeRangeId="'.$sizeRangeId.'",sizeRangeName="'.$sizeRangeName.'",description="'.$description.'",modifyDate="'.$modifyDate.'",modifyBy="'.$_SESSION['userid'].'"';
...
$whereCheck='name="'.$name.'" and materialTypeId="'.$materialTypeId.'"';

$duplicateYes = checkduplicate('materialSubType',$whereCheck);
if($duplicateYes!='yes'){
  ...
  ?>
  <script>
    alert('This name is already exist. Please enter new name!');
  </script>
  <?php
}
}
...
$facName=mysqli_fetch_array($aa);
   ...
   $update = updatelisting('styleSubCategoryMaster','approved="2",addedBy="'.$_SESSION['userid'].'"','id="'.$itennss.'"');
...
if(trim($_POST['action'])=='gateentrymaster' && isset($_POST["Submit1"])){
  ...
  $checkduplicateBill = checkduplicate('gateentrymaster','billno="'.clean($_POST['billno']).'"');

  if(clean($_POST['isUpdate']=='no')){
    if($checkduplicateBill=='yes'){
      ?>
      <script>
        alert('Bill number already exist. Please enter new bill number!');
      </script>
      <?php
      die();
    }
...
$rsnew=GetPageRecord('id','queryMaster','1 and styleRefId="'.$styleno1.'" and deletestatus=0');
...
if($_POST['editId']==""){
  $i=0;
  foreach($_POST['materialId'] as $datatosave){
    $kk = 'requisitionId="'.$requisition.'",styleId="'.$styleno.'",materialId="'.$_POST['materialId'][$i].'",color="'.$_POST['colorId'][$i].'",dateAdded="'.$dateAdded.'",dateCreated="'.$dateCreated.'",issueqty="'.$_POST['issueqty'][$i].'",balance="'.$_POST['balance'][$i].'",status="1"';
    addlisting('issuanceMaster',$kk);
  $i++;
  }
...
$materialMasterId=clean($_POST['materialMasterId']);
...
<script>
parent.setupbox(
    'showpage.crm?module=amendment&&view=yes&id=<?php echo encode($lasttid); ?>&styleid=<?php echo encode($styleId); ?>'
    );
</script>
<?php
...
<script>
parent.setupbox(
    'showpage.crm?module=<?php echo $_POST['module']; ?>&view=yes&id=<?php echo encode($id); ?>&styleid=<?php echo encode($styleId); ?>'
    );
</script>
<?php
...
$namevalue ='grnDate="'.$grnDate.'",challanNo="'.$challanNo.'",eWayBill="'.$eWayBill.'",docNo="'.$docNo.'",eWayBillDate="'.$eWayBillDate.'",supplierId="'.$supplierId.'",supplierGst="'.$supplierGst.'",departmentId="'.$departmentId.'",costCenter="'.$costCenter.'",paymentNo="'.$paymentNo.'",gateEntryNo="'.$gateEntryNo.'",attachment="'.$attachment.'",perfInvoice="'.$perfInvoice.'",grnId="'.$grnId.'"';
...
$aaa ='poSubTypeId="'.$poSubTypeId.'",styleId="'.$styleId.'",materialId="'.$materialId.'",supplierId="'.$supplierId.'",materialQty="'.$materialQty.'",orderQty="'.$orderQty.'",sellingRate="'.$sellingRate.'",bomWidth="'.$bomWidth.'",sellingValue="'.$sellingValue.'",pendingQty="'.$pendingQty.'",uom="'.$uom.'",rate="'.$rate.'",requisitionNo="'.$requisitionNo.'",status=1,dateAdded="'.time().'",createdDate="'.date('Y-m-d').'",materialTypeId=1,poTypeId="'.$poTypeId.'",GreigeYarn=1,addedBy="'.$addedBy.'",materialMasterId="'.$materialMasterId.'"';
...
$kk ='supplier="'.$supplier.'",address="'.$address.'",phone="'.$phone.'",gstn="'.$gstn.'",state="'.$state.'",stylenum="'.$stylenum.'",indentnum="'.$indentnum.'",process="'.$process.'",challantype="'.$challantype.'",personname="'.$personname.'",dispatch="'.$dispatch.'",date="'.$date.'",vehiclenum="'.$vehiclenum.'",gatenum="'.$gatenum.'",ewaynum="'.$ewaynum.'",factoryname="'.$factoryname.'",factoryaddress="'.$factoryaddress.'",factoryphone="'.$factoryphone.'",factorygstn="'.$factorygstn.'",factorystate="'.$factorystate.'",pono="'.$pono.'",status="1",departmentId="'.$departmentId.'"';
...
$utgstrate = clean($_REQUEST['utgstrate']);
$qtyIssued = clean($_REQUEST['qtyIssued']);
$qtyReceived = clean($_REQUEST['qtyReceived']);
$qtyRemain = clean($_REQUEST['qtyRemain']);

$where='id="'.decode($_REQUEST['id']).'"';

$namevalue ='itemcode="'.$itemcode.'",hsncode="'.$hsncode.'",reason="'.$reason.'",uom="'.$uom.'",qty="'.$qty.'",rate="'.$rate.'",amnt="'.$amnt.'",cgstrate="'.$cgstrate.'",cgstamt="'.$cgstamt.'",sgstrate="'.$sgstrate.'",sgstamt="'.$sgstamt.'",igstamt="'.$igstamt.'",igstrate="'.$igstrate.'",utgstrate="'.$utgstrate.'",utgstamt="'.$utgstamt.'",qtyIssued="'.$qtyIssued.'",qtyReceived="'.$qtyReceived.'",qtyRemain="'.$qtyRemain.'"';
...
$fds_cut_width = $_POST['fds_cut_width'];

$fds_ful_width= $_POST['fds_ful_width'];
...
shell_shrn_wid="'.$shell_shrn_wid.'", shell_patrn_len="'.$shell_patrn_len.'", shell_patrn_wid="'.$shell_patrn_wid.'", createdBy="'.$created_at.'",dateAdded=NOW(), fds_cut_width="'.$fds_cut_width.'", fds_ful_width="'.$fds_ful_width.'"';
...
shell_shrn_wid="'.$shell_shrn_wid.'", shell_patrn_len="'.$shell_patrn_len.'",shell_patrn_wid="'.$shell_patrn_wid.'", fds_cut_width="'.$fds_cut_width.'", fds_ful_width="'.$fds_ful_width.'"';
...
$mailsubject = "PO Received From: '".$systemname."'";

$maildescription = 'Dear Sir/Mam,</b> <br>

							<br>

							Greeting from '.$systemname.' - The Apparel ERP!</p>

<p>Please find the attached PO Number: '.$poNumber.' for your reference.</p>
<p>Remark: '.$remark.'</p>
...
$mailto = getSupplierEmail($supplierId);

$ccmail='';

$fromemail ='info@mail.com';
$attfilename = "images/".$file_name;

$newFileName = $poNumber.'.pdf';
send_template_mail_query_with_attachment($fromemail,$mailto,$mailsubject,$maildescription,$ccmail,$attfilename,$newFileName);

$namevalue = 'poMailSentStatus="1",remark="'.$remark.'"';
...
if(trim($_POST['action'])=='yarnrequisition' && trim($_POST['module'])!=''){
...
$indentNumber='';
$final_or_died_yarn=clean($_POST['final_or_died_yarn']);
...
$indentNumber = 'Y-IND-'.date('dmy').decode($_POST['editId']);

}



$namevalue ='requisitionNo="'.$requisitionNo.'",requisitionDate="'.$requisitionDate.'",brandId="'.$brandId.'",addedBy="'.$addedBy.'",status="'.$status.'",seasonId="'.$seasonId.'",indentStatus="'.$indentStatus.'",indentNumber="'.$indentNumber.'",styleNo="'.$styleNo.'",tabstatus="1",currencyId="'.$currencyId.'",final_or_died_yarn="'.$final_or_died_yarn.'"';
...
$update = updatelisting('yarnRequisition',$namevalue,$where);

}else{

$lasttid=addlistinggetlastid('yarnRequisition',$namevalue);
...
?>
<script>
parent.setupbox('showpage.crm?module=greigerequisition&alt=1');
</script>
<?php }
...
if($_REQUEST['action']=='yarnindentSendpo' && $_POST['sellingValue']!=0 && $_POST['sellingValue']!=0){
...
$aaa ='poSubTypeId="'.$poSubTypeId.'",styleId="'.$styleId.'",materialId="'.$materialId.'",uom="'.$uom.'",supplierId="'.$supplierId.'",materialQty="'.$materialQty.'",orderQty="'.$orderQty.'",sellingRate="'.$sellingRate.'",sellingValue="'.$sellingValue.'",pendingQty="'.$pendingQty.'",rate="'.$rate.'",requisitionNo="'.$requisitionNo.'",status=1,dateAdded="'.time().'",createdDate="'.date('Y-m-d').'",materialTypeId=1,poTypeId="'.$poTypeId.'",GreigeYarn=1,addedBy="'.$addedBy.'",materialMasterId="'.$materialMasterId.'"';

$adds = addlisting('indentCreationMaster',$aaa);

?>
<script>
parent.setupbox('showpage.crm?module=yarnindent&add=yes&id=<?php echo $_POST['pageeditid']; ?>');
</script>
<?php

}

if(trim($_POST['action'])=='yarnallocation' && trim($_POST['module'])!=''){
...
$lasttid=addlistinggetlastid('yarnAllocation',$namevalue);
...
?>
<script>
parent.setupbox('showpage.crm?module=greigeallocation&alt=1');
</script>
<?php }
...
$requisitionNo = $_POST['requisitionNo'];
$color = $_POST['color'];
$oldMaterialId = $_POST['oldMaterialId'];
$addedBy = $_SESSION['userid'];
$materialMasterId = $_POST['materialId'];
$isFinal = $_POST['isFinal'];

$aaa ='poSubTypeId="'.$poSubTypeId.'",styleId="'.$styleId.'",materialId="'.$materialId.'",supplierId="'.$supplierId.'",materialQty="'.$materialQty.'",orderQty="'.$orderQty.'",sellingRate="'.$sellingRate.'",bomWidth="'.$bomWidth.'",sellingValue="'.$sellingValue.'",pendingQty="'.$pendingQty.'",uom="'.$uom.'",rate="'.$rate.'",requisitionNo="'.$requisitionNo.'",status=1,dateAdded="'.time().'",createdDate="'.date('Y-m-d').'",materialTypeId=1,poTypeId="'.$poTypeId.'",GreigeYarn=1,final_or_died_yarn="2",parentId="'.$parentId.'",color="'.$color.'",oldMaterialId="'.$oldMaterialId.'",addedBy="'.$addedBy.'",materialMasterId="'.$materialMasterId.'",isFinal="'.$isFinal.'"';

if($_POST['editId']==""){
	$adds = addlisting('indentCreationMaster',$aaa);
}else{
	echo $where = 'id="'.$_POST['editId'].'"';
	$update = updatelisting('indentCreationMaster',$aaa,$where);
...
if(trim($_POST['action'])=='colorcardmasterfromsyle' && trim($_POST['editId'])=='' && trim($_POST['name'])!='' && trim($_POST['module'])!=''){
...
if(trim($_POST['action'])=='sizemaster' && trim($_POST['name'])!='' && trim($_POST['module'])!=''){
  ...
  if(trim($_POST['editId'])==''){
    $namevalue ='name="'.$name.'",description="'.$description.'",dateAdded="'.$dateAdded.'",addedBy="'.$_SESSION['userid'].'"';
    $adds = addlisting('sizeMaster',$namevalue);
  }else{
    $namevalue ='name="'.$name.'",description="'.$description.'",modifyDate="'.$dateAdded.'",modifyBy="'.$_SESSION['userid'].'"';

    $where = $where='id='.decode($_POST['editId']).'';
    $update = updatelisting('sizeMaster',$namevalue,$where);
  }
  ?>
  <script>
  parent.setupbox('showpage.crm?module=<?php echo $_POST['module']; ?>&alt=1');
...
if(trim($_POST['action'])=='uoloadopeningbalance'){
  ...
  $data->read($path);
    ...
    for($x = 2; $x <= count($data->sheets[0]["cells"]); $x++) {
      $materialCode = trim(addslashes($data->sheets[0]["cells"][$x][1]));
      $balance = trim(addslashes($data->sheets[0]["cells"][$x][2]));

      if($materialCode!='' && $balance!=''){

        $poNum=GetPageRecord('balance','materialMaster','materialUniqueId="'.$materialCode.'"');
        $poNumDetail=mysqli_fetch_array($poNum);
        $balance = $balance+$poNumDetail['balance'];

        $namevalue = 'balance="'.$balance.'"';
        $where = 'materialUniqueId="'.$materialCode.'"';
        $update = updatelisting('materialMaster',$namevalue,$where);

        $_SESSION['error'] = "File Uploade Successful!";
      }
...
if(trim($_POST['action'])=='widthmaster' && trim($_POST['editId'])=='' && trim($_POST['name'])!='' && trim($_POST['module'])!=''){
  ...
  $adds = addlisting('widthMaster',$namevalue);
  ...
  $update = updatelisting('widthMaster',$namevalue,$where);
...
if(trim($_POST['action'])=='addactivity' && trim($_POST['activityId'])!=''){

$activityId=clean($_POST['activityId']);
$plannedDate=date('Y-m-d',strtotime($_POST['plannedDate']));
$styleId=decode($_POST['styleId']);
$colorId=clean($_POST['colorId']);
$status='0';
$dateAdded=time();

$namevalue ='activityId="'.$activityId.'",plannedDate="'.$plannedDate.'",status="'.$status.'",dateAdded="'.$dateAdded.'",styleId="'.$styleId.'",colorId="'.$colorId.'"';
addlistinggetlastid('samplingActivityMaster',$namevalue);
...
if(trim($_POST['action'])=='editactivity' && trim($_POST['dispatch'])!=''){


	$dispatch=clean($_POST['dispatch']);
	$styleId=decode($_POST['styleId']);
	$actualDate=date('Y-m-d');
	$wip = $_POST['wip']-$dispatch;

	$totalDispatch = $dispatch+clean($_POST['totalDispatch']);

	if(clean($_POST['totalWip'])==$totalDispatch){
		$status='1';
	}


	$namevalue ='wip="'.$wip.'",status="'.$status.'",actualDate="'.$actualDate.'",totalDispatch="'.$totalDispatch.'"';

	$where='id="'.decode($_POST['editId']).'"';
	$update = updatelisting('samplingActivityMaster',$namevalue,$where);
  ...
  if(trim($_REQUEST['action'])=="loadstyleworkflow" && $_REQUEST['id']!=""){
    ...
    $operatorId=trim($_REQUEST['operatorId']);
    $userId=trim($_REQUEST['userId']);

    $uom=trim($_REQUEST['uom']);

    $checkVal=trim($_REQUEST['checkVal']);

    $dateAdded = date('Y-m-d');

    $namevalue ='sequenceNo="'.$sequenceNo.'",departmentId="'.$departmentId.'",operationId="'.$operationId.'",operatorId="'.$operatorId.'",uom="'.$uom.'",checkVal="'.$checkVal.'",dateAdded="'.$dateAdded.'",userId="'.$userId.'"';

    $where='id="'.$id.'"';

    $update = updatelisting('styleWorkFlowMaster',$namevalue,$where);
...
if(trim($_POST['action'])=='saveoperatorinput' && trim($_POST['styleId'])!=''){

// echo '<pre>'; print_r($_POST); die();
  $fromDate=date("Y-m-d", strtotime($_POST['fromDate']));
  $factoryId=addslashes($_POST['factoryId']);
  $stockType=addslashes($_POST['stockType']);
  $line=addslashes($_POST['line']);
  $styleId=decode(addslashes($_POST['styleId']));

  $receivedFrom=addslashes($_POST['receivedFrom']);

  $receivedQty=addslashes($_POST['receivedQty']);

  $transferTo=addslashes($_POST['transferTo']);

  $transferQty=addslashes($_POST['transferQty']);

  $sequenceNo=addslashes($_POST['sequenceNo']);

////////////////////For check quality//////////////
  $totalCheck=addslashes($_POST['totalCheck']);

  $pass=addslashes($_POST['pass']);

  $fail=addslashes($_POST['fail']);

  $transferTo2=addslashes($_POST['transferTo2']);

  $transferQty2=addslashes($_POST['transferQty2']);
////////////////////End here////////////////////////

  //$namevalue ='fromDate="'.$fromDate.'",factoryId="'.$factoryId.'",line="'.$line.'",receivedFrom="'.$receivedFrom.'",receivedQty="'.$receivedQty.'",transferTo="'.$transferTo.'",transferQty="'.$transferQty.'",addedBy="'.$_SESSION['userid'].'",styleId="'.$styleId.'",sequenceNo="'.$sequenceNo.'",stockType="'.$stockType.'"';

  //addlisting('operatorInput',$namevalue);

  if($totalCheck!='' && $transferQty2!=''){

    $namevalue2 ='fromDate="'.$fromDate.'",factoryId="'.$factoryId.'",line="'.$line.'",pass="'.$pass.'",fail="'.$fail.'",transferTo="'.$transferTo2.'",transferQty="'.$transferQty2.'",addedBy="'.$_SESSION['userid'].'",styleId="'.$styleId.'",sequenceNo="'.$sequenceNo.'",stockType="'.$stockType.'",totalCheck="'.$totalCheck.'",dateAdded="'.time().'"';
...
$sno = 1;
while($sno <= $_POST['addcount']){
    $orderWiseNo=trim($_POST["orderWiseNo".$sno]);
    $ticketNo=trim($_POST["ticketNo".$sno]);
    $color=trim($_POST["color".$sno]);
    $size=trim($_POST["size".$sno]);
    $remark = implode(",",$_POST["remark".$sno]);

    $namevalue3 ='orderWiseNo="'.$orderWiseNo.'",ticketNo="'.$ticketNo.'",color="'.$color.'",size="'.$size.'",operatorId="'.$transferTo2.'",remark="'.$remark.'",styleId="'.$styleId.'",operatorInputId="'.$lastid.'",dateAdded="'.time().'",addedBy="'.$_SESSION['userid'].'"';
...
?>
<script>
parent.setupbox('showpage.crm?module=newcoa&alt=1');
</script>
<?php
}

if($_POST['action']=="accountsubgroup"){
...
if($_POST['action']=="accountnamemaster"){
...
if(trim($_POST['action'])=='othersmaster' && trim($_POST['name'])!='' && trim($_POST['code'])!='' && trim($_POST['module'])!=''){
  ...
  if($_POST['editId']!=''){
    $where='id='.decode($_POST['editId']).'';
    $namevalue ='name="'.$name.'",code="'.$code.'",status="'.$status.'",modifyDate="'.$modifyDate.'",modifyBy="'.$_SESSION['userid'].'"';
    $update = updatelisting('othersMaster',$namevalue,$where);
  }else{
    $namevalue ='name="'.$name.'",code="'.$code.'",status="'.$status.'",dateAdded="'.$dateAdded.'",addedBy="'.$_SESSION['userid'].'"';
    $lastId = addlistinggetlastid('othersMaster',$namevalue);
  }
