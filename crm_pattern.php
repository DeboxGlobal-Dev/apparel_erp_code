...
?>
                      <tr role="row" class="odd">
                        <td><a href="showpage.crm?module=pattern&add=yes&styleid=<?php echo encode($resultlists['id']); ?>"><?php echo '#'.$resultlists['styleRefId']; ?>



                          </a></td>
                        <td><?php echo $resultlists['subject']; ?></td>
						 <td><?php echo getUserName($resultlists['patternAddedBy']); ?></td>
                        <td><?php if($resultlists['patternAddDate']=='0'){ echo '-';  }else{ echo date('d M, Y - h:i A',$resultlists['patternAddDate']); }  ?></td>
					    ...
					    <td><span class="badge" style="cursor:pointer;background-color:green; color:#fff; position: relative;width: 142px; font-size: 11px; padding: 6px;"  onclick="opmodalpop('Add Pattern','modalpop.php?action=uploadatechpackpattern&styleId=<?php echo encode($resultlists['id']); ?>&module=<?php echo $_GET['module']; ?>','600px','auto');" data-toggle="modal" data-target="#modalpop" >
									Add&nbsp;Pattern</span></td>
                        <td align="center"><?php if($resultlists['patternAttachment']!=''){ ?>
                          <a href="images/<?php echo $resultlists['patternAttachment']; ?>" target="blank">&nbsp;&nbsp;<i class="fa fa-download" aria-hidden="true" style="display: inherit;margin-right: 5px;"></i>Download</a>
                          <?php } ?></td>
                      </tr>
                      <?php } ?>
                    </tbody>
