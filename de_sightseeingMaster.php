					...
					<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer"> <div class="datatable-scroll"><table class="table table-bordered table-hover datatable-highlight dataTable no-footer" id="DataTables_Table_2" role="grid" aria-describedby="DataTables_Table_2_info">
						<thead>
							<tr role="row">
							  ...
							  <th class="sorting_asc" tabindex="0" aria-controls="DataTables_Table_2" rowspan="1" aria-sort="ascending" aria-label="First Name: activate to sort column descending">Supplier</th>
							  <th class="sorting" tabindex="0" aria-controls="DataTables_Table_2" rowspan="1" aria-label="Last Name: activate to sort column ascending">Cost (INR) </th>
							  <th class="sorting" tabindex="0" aria-controls="DataTables_Table_2" rowspan="1" colspan="1" aria-label="Job Title: activate to sort column ascending">Location</th>
							  <th class="sorting" tabindex="0" aria-controls="DataTables_Table_2" rowspan="1" colspan="1" aria-label="Status: activate to sort column ascending">Duration</th>
							  <th class="sorting" tabindex="0" aria-controls="DataTables_Table_2" rowspan="1">Star&nbsp;Rating</th>
							  <th class="sorting" tabindex="0" aria-controls="DataTables_Table_2" rowspan="1">Update&nbsp;Date</th>
							  <th class="sorting" tabindex="0" aria-controls="DataTables_Table_2" rowspan="1">Update&nbsp;By</th>
							  <th class="sorting" tabindex="0" aria-controls="DataTables_Table_2" rowspan="1">Status</th>
							</tr>
						</thead>
						<tbody>
...
$where='where  '.$wheresearchassign.' '.$wheresearch.' and name!="" '.$assignto.'  order by updateDate desc';
$page=$_GET['page'];

$targetpage=$fullurl.'showpage.crm?module=sightseeingMaster&records='.$limit.'&searchField='.$searchField.'&assignto='.$_GET['assignto'].'&';
$rs=GetRecordList($select,'sightseeingMaster',$where,$limit,$page,$targetpage);
$totalentry=$rs[1];
$paging=$rs[2];
...
?>
						...
						<tr role="row" class="odd">
								<td class="sorting_1"><a class="shorttextcard"  title="<?php echo $resultlists['name']; ?>" style=" color:#2196f3; display:block; " href="#" onclick="view('<?php echo encode($resultlists['id']); ?>');" ><?php echo strip($resultlists['name']); ?></a></td>
								<td class="sorting_1"><?php
										$select1='*';
									    $where1='id="'.$resultlists['supplier'].'"';
										$rs1=GetPageRecord($select1,_VENDOR_MASTER_,$where1);
										$editresult2=mysqli_fetch_array($rs1);
										echo $vendortype= $editresult2['name'];
								?></td>
								<td class=""><?php echo strip($resultlists['cost']); ?></td>
								<td><?php
													$select1='*';
													$where1='id="'.$resultlists['destinaton'].'"';
													$rs1=GetPageRecord($select1,'destinationMaster',$where1);
													$editresult2=mysqli_fetch_array($rs1);
													echo $editresult2['name'];
													?>, <?php if(!empty($resultlists['country'])){ echo getCountryName($resultlists['country']);} ?></td>
								<td><a class="shorttextcard" title="<?php echo $resultlists['email']; ?>" href="mailto:<?php echo $resultlists['email']; ?>"  style="color:#2196f3; display:block;"><?php echo $resultlists['duration']; ?></a></td>
								<td class="text-left"><span class="shorttextcard"><?php echo  ($resultlists['starRating']).'&nbsp;Star';?></span></td>
							    <td class="text-left"><span class="shorttextcard"><?php echo date('d-m-Y H:i:a',$resultlists['updateDate']); ?></span></td>
							    <td class="text-left"><span class="shorttextcard"><?php echo  getUserName($resultlists['updateBy']);	?></span></td>
							    <td class="text-left"><span class="shorttextcard"><?php if($resultlists['status']==0){ echo "Active"; }else{ echo "Inactive"; } ?></span></td>
						</tr>
