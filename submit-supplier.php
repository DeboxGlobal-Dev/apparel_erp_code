...
<?php
$selecty='*';
$wherey='styleid="'.decode($_GET['st']).'" and supplierid="'.decode($_GET['s']).'"';
$rsy=GetPageRecord($selecty,'supplierPurchasemail',$wherey);
...
<?php
$select='*';
$where='styleId="'.decode($_GET['st']).'" and costsheetVersionId="'.decode($_GET['cv']).'" and supplierPurchaseEmailId="'.decode($_GET['emid']).'" group by materialTypeId asc';
                          ...
                          <tbody style="width: 100%;display: inline-table;">
                            <tr class="card-body">
							 <td width="18%" align="left" style="border:1px solid #ccc;"><strong>Name</strong></td>
							  ...
							  <td width="18%" align="left" style="border:1px solid #ccc;"><strong>Avg.</strong></td>
							  <td width="18%" align="left" style="border:1px solid #ccc;"><strong>Unit</strong></td>
							  <td width="41%" align="left" style="border:1px solid #ccc;"><strong>Comment</strong></td>
...
$selectmid='*';
if($_GET['p']=='1'){
$wheremid='styleId="'.decode($_GET['st']).'" and costsheetVersionId="'.decode($_GET['cv']).'" and materialTypeId="'.$resListing['materialTypeId'].'" and supplierPurchaseEmailId="'.decode($_GET['emid']).'" group by materialId';
}else{
$wheremid='styleId="'.decode($_GET['st']).'" and costsheetVersionId="'.decode($_GET['cv']).'" and supplierId="'.decode($_GET['s']).'" and materialTypeId="'.$resListing['materialTypeId'].'" and supplierPurchaseEmailId="'.decode($_GET['emid']).'" order by id asc';
}
...
$select1='*';
$where1='id="'.$resListingmid['materialId'].'" and styleId="'.decode($_GET['st']).'" and costsheetVersionId="'.decode($_GET['cv']).'"';
$rs1=GetPageRecord($select1,'styleSubCategoryMaster',$where1);
$resListing1=mysqli_fetch_array($rs1);

$rowno++;
$sNo1=$rowno;
$rs121=GetPageRecord('*','techPackDetailMaster',' bomSerialNo="'.$sNo1.'" and sectionType="bom" and styleId="'.decode($_GET['st']).'" and costsheetVersionId=1 order by id asc');
...
?>
		<tr class="card-body" style="color: #666;">
			...
			<td align="left" style="border:1px solid #ccc;"><?php echo $resListing12['bomAvg']; ?></td>
			<td align="left" style="border:1px solid #ccc;"><?php echo $resListing12['bomUnit']; ?></td>
			<td align="left" style="border:1px solid #ccc;"><?php echo $resListingmid['inhouseRemark']; ?></td>
			<td align="left" style="border:1px solid #ccc;">
			<input class="quantity" name="valueOnePiece" type="text" id="valueOnePiece<?php echo $resListingmid['id']; ?>" onkeyup="submitSupplierData<?php echo $resListingmid['id']; ?>();" autocomplete="off" style="width: 100px;text-align: left;border: 1px solid #ccc;padding: 3px;" value="<?php echo $resListingmid['valueOnePiece']; ?>" <?php if($resListingmid['valueOnePiece']!='' && $resListingmid['valueOnePiece']!='0') { ?> disabled <?php } ?> />
			</td>
			<td align="left" style="border:1px solid #ccc;">
			<input name="supplierRemark" type="text" id="supplierRemark<?php echo $resListingmid['id']; ?>" onkeyup="submitSupplierData<?php echo $resListingmid['id']; ?>();" autocomplete="off" style="width: 200px;text-align: left;border: 1px solid #ccc;padding: 3px;"  value="<?php echo $resListingmid['supplierRemark']; ?>"  <?php if($resListingmid['supplierRemark']!='') { ?> disabled <?php } ?> />
			</td>
		</tr>

<script>
function submitSupplierData<?php echo $resListingmid['id']; ?>(){
	var valueOnePiece = Number($('#valueOnePiece<?php echo $resListingmid['id']; ?>').val());
	var supplierRemark = encodeURI($('#supplierRemark<?php echo $resListingmid['id']; ?>').val());
	$('#submitsupplierdata').load('outsourceaction.php?id=<?php echo encode($resListingmid['id']); ?>&action=sendtosupplier&valueOnePiece='+valueOnePiece+'&supplierRemark='+supplierRemark);
}
</script>
...
<?php
$selecty1='*';
$wherey1='styleid="'.decode($_GET['st']).'" and supplierid="'.decode($_GET['s']).'" and id="'.decode($_GET['emid']).'" and status=1 order by id desc';
$rsy1=GetPageRecord($selecty1,'supplierPurchasemail',$wherey1);
...
<input type="hidden" name="action" value="supplierattachment"  />
