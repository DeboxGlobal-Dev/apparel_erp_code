                ...
                <div class="datatable-scroll">
                  <table class="table table-bordered table-hover no-footer">
                    <thead style="background-color: #f5f5f5;">
                      <tr role="row">
                        <th><span class="sorting">Style#</span></th>
                        <th>Style&nbsp;Name</th>
						<th>Job&nbsp;Done&nbsp;By</th>
                        <th>Prepared&nbsp;Date</th>
						<th>Assign&nbsp;To </th>
...
$rs1ssss=GetPageRecord('*','techpackPatternMarkerUpload','1 and styleId="'.$resultlists['id'].'" and sectionType="'.$_GET['module'].'" order by id desc');
$result1ss=mysqli_fetch_array($rs1ssss);


?>
                      <tr role="row" class="odd">
                        <td><a href="showpage.crm?module=marker&add=yes&styleid=<?php echo encode($resultlists['id']); ?>"><?php echo '#'.$resultlists['styleRefId']; ?>



                          </a></td>
                        <td><?php echo $resultlists['subject']; ?></td>
						 <td><?php echo getUserName($resultlists['markerAddedBy']); ?></td>
                        <td><?php if($resultlists['markerAddDate']=='0'){ echo '-';  }else{ echo date('d M, Y - h:i A',$resultlists['markerAddDate']); }  ?></td>
                        <td><?php if($loginuserprofileId==90){ echo getUserName($resultlists['assignToVendor']); }else{ echo getUserName($resultlists['assignTo']); }?></td>
                         <td><span class="badge" style="cursor:pointer;background-color:green; color:#fff; position: relative;width: 142px; font-size: 11px; padding: 6px;"  onclick="opmodalpop('Add Marker','modalpop.php?action=uploadatechpackpattern&styleId=<?php echo encode($resultlists['id']); ?>&module=<?php echo $_GET['module']; ?>','600px','auto');" data-toggle="modal" data-target="#modalpop" >
									Add&nbsp;Marker</span></td>
                        <td align="center"><?php if($result1ss['attachtp']!=''){ ?>
                          <a href="images/<?php echo $result1ss['attachtp']; ?>" target="blank">&nbsp;&nbsp;<i class="fa fa-download" aria-hidden="true" style="display: inherit;margin-right: 5px;"></i>Download</a>
                          <?php } ?></td>
                      </tr>
                      <?php } ?>
                    </tbody>
