...
$rs=GetPageRecord('*','styleWorkFlowMaster','id="'.decode($_GET['id']).'"');
...
<tr
	style="padding: 5px; font-size: 13px; font-weight: 600; background-color: #F8F8F8; margin-top: 0; position: relative;">
	<td align="center">
		<div>Factory</div>
		<p style="color:green;">
		<?php
	...
	<td align="center">
		<?php
		$rkdm=GetPageRecord('min(uploadInputDate) as minDate, max(uploadInputDate) as maxDate','linePlanMaster','1 and styleId="'.$editresultstyle['id'].'"');
		...
		?>
		<div>From Date</div>
		<p style="color:green;"><?php if($startDate!='01-01-1970'){ echo $startDate; }; ?></p>
	</td>
	<td align="center">
		<div>To Date</div>
		<p style="color:green;"><?php if($endDate!='01-01-1970'){ echo $endDate; }; ?></p>
	</td>
</tr>
...
<tr
	...
	<td align="center">
		<div><i class="icon-add" style="font-size:18px;cursor:pointer;"
				onclick="addnewrow(1);"></i></div>
	</td>
...
function addnewrow(id) {
	...
	if (id == 1) {
		...
		$("#loadrow").load(
			'loadaction.php?add=1&action=loadstyleworkflow&styleId=<?php echo encode($styleId); ?>'
			);

	} else {

		$('#loadrow').load(
			'loadaction.php?action=loadstyleworkflow&styleId=<?php echo encode($styleId); ?>'
			);
...
function deleteRow(id) {
	...
	if (checkyes == true) {
		...
		$('#loadrow').load('loadaction.php?id=' + id +
			'&deletestatus=yes&action=loadstyleworkflow&styleId=<?php echo encode($styleId); ?>'
			);
...
onClick="window.location.reload();">Save<i class="fa fa-floppy-o ml-2"
