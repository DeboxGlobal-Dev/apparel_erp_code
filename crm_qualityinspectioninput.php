	...
	<?php } ?>
					    ...
					    <form action="ac.crm" method="post" enctype="multipart/form-data" name="popid" target="acf" id="popid">
									 	...
									 	<input name="action" type="hidden" id="action" value="savequalityinspect" />
								...
								<div class="card-body">
									...
									<input name="fromDate" type="text" class="form-control" id="fromDate" value="<?php if($editresult['fromDate']!=''){ echo date('d-m-Y', strtotime($editresult['fromDate'])); }else{ echo date('d-m-Y'); } ?>" readonly="">
									</div>
									...
									<option value="">Select Factory</option>
 									<?php
									$a=GetPageRecord('*','factoryMaster','1 order by name asc');
									while($factoryData=mysqli_fetch_array($a)){ ?>
									...
									<div id="loadrecorderinputlines">
										<select class="form-control" name="line" id="line" onchange="selectDataStyle();">
									...
									$('#loadrecorderinputlines').load('loadlinessingle.php?id='+id+'&selectId=<?php echo $_GET['lines']; ?>');
									 }
									</script>
								...
								<div class="card-body" style="border:0px !important;">
										...
										<span class="text-muted">Number of Defects Found</span><br><input type="text" name="numberofdefects" id="numberofdefects" >
										</div>
										<div class="media-body">
										<span class="text-muted">Number of Pass Garments</span><br>
										<input type="text" name="numberofpassgar" id="numberofpassgar" onkeyup="caltotal();">
										</div>
										<div class="media-body">
										<span class="text-muted">Number of Defective Garments</span>
										<input type="text" name="numberofdefgar" id="numberofdefgar" onkeyup="caltotal();">
										</div>
										<div class="media-body">
										<span class="text-muted">Total pieces Checked</span>
										 <input type="text" name="totalpricechck" id="totalpricechck" readonly="" >
										</div>
										<div class="media-body">
			 ...
			 <script>
			 function caltotal(){
			var numberofpassgar=$('#numberofpassgar').val();
			var numberofdefgar=$('#numberofdefgar').val();
		 	var totalpricechcka=Number(numberofpassgar)+ Number(numberofdefgar);
			totalpricechcka= parseFloat(totalpricechcka).toFixed(2);

			$('#totalpricechck').val(totalpricechcka);
			 }
			 </script>
...
.recorder-input{
width:100%; margin:auto; border:0px !important;
}
