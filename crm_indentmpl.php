...
?>
							 ...
							 <td align="center"><?php echo $resultlists['displayId']; ?></td>
								<td><a href="showpage.crm?module=indentmpl&add=yes&styleid=<?php echo encode($resultlists['id']); ?>&indentno=<?php echo encode($resultqty['indentNumber']); ?>"><?php echo $resultqty['indentNumber']; ?></a></td>

								<td><?php echo '#'.$resultlists['styleRefId']; ?><?php if(countQueryunreadMails($resultlists['id'])!=0){ ?><div class="numberbubbol"><?php echo countQueryunreadMails($resultlists['id']); ?></div><?php } ?></td>
						...
						<!--<a href="showpage.crm?module=indentmpl&add=yes&styleid=<?php echo encode($resultlists['id']); ?>" target="_blank" style="background: green; outline: none; color: #fff; padding: 5px; border-radius: 2px; cursor: pointer; width: 95px; display: block; float:left; text-align:center;">View</a> -->
						<a href="download-indent.php?styleid=<?php echo encode($resultlists['id']); ?>" target="_blank" style="background: #0288d1; outline: none; color: #fff; padding: 5px; border-radius: 2px; cursor: pointer; width: 95px; display: block; float:left;  text-align:center;"><i class="fa fa-download" aria-hidden="true"></i> Excel</a>
						<a href="tcpdf/examples/genrateindent.php?pageurl=<?php echo $fullurl; ?>indentpdf.php?styleid=<?php echo encode($resultlists['id']); ?>&indentno=<?php echo encode($resultqty['indentNumber']); ?>" target="_blank" style="background: #e71c22; outline: none; color: #fff; padding: 5px; border-radius: 2px; cursor: pointer; width: 95px; display: block; float:right; margin-left:5px; text-align:center;"><i class="fa fa-download" aria-hidden="true"></i> PDF</a>
						</div>


						<!--<a href="showpage.crm?module=indentmpl&add=yes&styleid=<?php //echo encode($resultlists['id']); ?>">-->
						<!--		    <span class="badge" style="cursor:pointer;background-color:green; color:#fff; position: -->
						<!--		    relative;width: 142px; font-size: 11px; padding: 6px;">View Indent</span></a>-->
