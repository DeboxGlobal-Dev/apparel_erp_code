...
?>
						...
						<td align="left"><div align="left"><?php echo '#'.$resultlists['styleRefId']; ?></div></td>
                          ...
                          <td><div>
						<?php
						$where12 = '1 and styleId="'.$resultlists['id'].'"  and parentId in (select id from grnMaster where grnNo!="") and materialId in (select id from styleSubCategoryMaster where materialtype=1) order by id asc';
						$grnDataq=GetPageRecord('*','grnMaster',$where12);
						$lotNo=0;
						while($grnData=mysqli_fetch_array($grnDataq)){

						++$lotNo;

$countqualitymoduleq=GetPageRecord('id','qualitymodulemaster','1 and styleId="'.$resultlists['id'].'" and costsheetVersionId=1 and status=1 and deletestatus=0 and lotNoMaster="'.$lotNo.'" order by id asc');
$countqualitymodule=mysql_num_rows($countqualitymoduleq);
if($countqualitymodule>0){
						 ?>
