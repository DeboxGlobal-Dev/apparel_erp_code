...
if(isset($_POST['addbranchinfo'])){
  ...
  $url = $serverurlapi."vouchers/addBankReceiptAPI.php";
  $response = postCurlData($url,$jsonData);
  $res = json_decode($response,true);


  logger($InfoMessage." Saving addBankReceiptAPI.. ".$response);
...
<script>
        ...
        $(document).ready(function () {
            $(".add-row").click(function () {
				...
				$no=1;
				foreach($accountData->AccountNameData as $resultList){
				?><option value='<?php echo $resultList->AccountCode; ?>'><?php echo $resultList->AccountName; ?> [<?php echo $resultList->AccountCode; ?>]</option><?php } } } ?></select></td><td><input type='number' name='credit[]' id='credit_"+rowno+"' onkeyup='funcCreditEnable("+rowno+");' onBlur='funcSumValue();'  class='creditsum form-control' value='0' ></td><td><input type='text' name='narration[]' id='narration_"+rowno+"' class='v form-control'  value=''></td></tr>";
