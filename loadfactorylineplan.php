...
<div style="padding:10px; font-size:16px; font-weight:600; background-color:#F8F8F8; margin-top:10px; position: relative;"><?php

$totaldays=(strtotime($_GET['endDate']) - strtotime($_GET['startDate']))/60/60/24;
if($totaldays==1){ $totaldays=$totaldays; } else { $totaldays=$totaldays+1; }
...
?>
<td colspan="4"><div style="padding:5px;"><?php echo $lineData['lineName']; ?></div>
...
if($getlinedate!=''){
$abccccc = date('Y-m-d', strtotime("+1 day", strtotime($getlinedate['uploadInputDate'])));
}
else
{
...
$rrquerey=GetPageRecord('efficiency','slabMaster','1 and parentId="'.$smvname['id'].'" order by days desc');
$smvdayefficiencyquery=mysqli_fetch_array($rrquerey);
...
$mainvaluedaywise=($capacity*$smvdayefficiencyquery['efficiency'])/100;
}

$finalvalue=$finalvalue+$mainvaluedaywise;

$totalstylequantity=$totalstylequantity-$mainvaluedaywise;
$totalallot=$totalstylequantity-$mainvaluedaywise;
...
${"line" . $lineData['id']} .= '@@@ insert into linePlanMaster set styleId="'.$_GET['styleid'].'",uploadInputDate="'.$uploadInputDate.'",factoryId="'.$_REQUEST['factoryId'].'",lineId="'.$lineData['id'].'",dateWiseLineInput="'.round($mainvaluedaywise).'",linewiseefficiency="'.round($finalvalue).'"';
}
}
...
<td <?php if (strpos($totalholidays, $currdataVal) !== false) { ?> style="background-color: #000; color: #fff;" <?php }  ?>>
<div style="min-height:49px;vertical-align: middle;padding-top: 18px; width:70px;"><?php echo $dt->format("d-m-Y");  ?></div>
</td>
<td width="10%" style="color:#FFFFFF;<?php if($countrow!=0){ if (strpos($totalholidays, $currdataVal) === false) { ?>border:2px solid #ff0000;<?php }} ?><?php if (strpos($totalholidays, $currdataVal) !== false) {    ?>background-color: #000; color: #fff; <?php }  ?>" bgcolor="<?php if($countrow==0){ if($totalstylequantity>0 || $abcfinalcheck==1){

echo $editresultstyle['styleColor'];

  } else { ?>#F8FFC1<?php } } else {  echo $queryData['styleColor']; }  ?>" ><div style="width:120px;"><?php if($countrow!=0){ if (strpos($totalholidays, $currdataVal) === false) { echo $queryData['subject']; } } else {  if($totalstylequantity>0 || $abcfinalcheck==1){

if (strpos($totalholidays, $currdataVal) === false) { echo $editresultstyle['subject']; }

if (strpos($totalholidays, $currdataVal) === false) { $finalavg= ++$countforavg; }
  } }   ?></div></td>

<td align="center" style="border: 1px solid #ece9e9;<?php if (strpos($totalholidays, $currdataVal) !== false) {    ?>background-color: #000; color: #fff; <?php }  ?>">
...
<td align="center" style="border: 1px solid #ece9e9;<?php if (strpos($totalholidays, $currdataVal) !== false) {    ?>background-color: #000; color: #fff; <?php }  ?>"><div style="width:50px;"><?php if($countrow==0){ if (strpos($totalholidays, $currdataVal) === false) { echo round($finalvalue); } } ?></div></td>
<?php $startDate++;
$totalallot; } ?>
</tr>
