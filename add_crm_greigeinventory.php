					...
					<div class="table-responsive">
			 			<table width="100%" class="table table-bordered table-responsive" style="margin-bottom:5px;">
                          <tbody style="width: 100%;display: inline-table;">
                            <tr class="card-body">
                              <td width="100%" style="text-align:center;"><strong style="font-size:23px;">Greige Inventory </strong></td>
							</tr>
                          </tbody>
                        </table>
						<table width="100%" class="table table-bordered table-responsive" style="margin-bottom:5px;">
...
<a onclick="opmodalpop('Assign To Sourcing','modalpop.php?action=assigntosoursingteam&styleId=<?php echo $_GET['styleid']; ?>&costsheetVersionId=1','400px','auto');" data-toggle="modal" data-target="#modalpop" class="btn bg-teal-400" aria-expanded="false" style="border-radius: 2px; background-color: #0d7544; margin: 0px !important; padding: 5px 10px; font-size: 12px; font-weight: 500;"><i class="fa fa-plus" aria-hidden="true"></i> Assign To</a>
						...
						$tillTotalMaterialQty=0;
						$rsindent=GetPageRecord('SUM(orderQty) as tillorderQty,orderQty,materialQty,final_or_died_yarn,id,poNumber,isFinal','indentCreationMaster','requisitionNo="'.$requisitionNo.'" and materialId="'.$rslistint['materialId'].'"  and id="'.$rslistint['indentCreationId'].'"');
						$resListingIndent1=mysqli_fetch_array($rsindent);
						//$resListingIndent1['tillorderQty'];
						$tillTotalMaterialQty= $rslistnew['finalQty']-$resListingIndent1['tillorderQty'];
						...
						//echo ' parentId in (select id from greigeRequisition where requisitionNo="'.decode($_GET['greReq']).'") and srinkageId="'.$rslistint['materialId'].'"';
						$allocationNo=GetPageRecord('SUM(requestedQty) AS allocateQty','greigeRequisition',' parentId in (select id from greigeRequisition where requisitionNo="'.decode($_GET['greReq']).'") and srinkageId="'.$rslistint['materialId'].'" and addedToIndent=1');
				...
				// 			$wherethis='id="'.$rslistint['materialId'].'"';
						...
						<td><?php echo getColorName($rslistint['color']); ?></td>
							...
							<td><?php echo $rslistnew['processWidth']; ?></td> -->
							 <td><?php echo $rslistint['netReceived']; ?></td>
							...
							<td><?php
							$whIns = 'styleId="'.$lastId.'" and materialid="'.$rslistint['materialId'].'" and colorid="'.$rslistint['color'].'"';
							$rsins=GetPageRecord('acceptedField,id','lotWiseData',$whIns);
							$resListingIns=mysqli_fetch_array($rsins);
							$insQty = stripslashes($resListingIns['acceptedField']);
							echo stripslashes($resListingIns['acceptedField']);
							?></td>
							<td>
							<?php
							/*$requestedQty = 0;
							$rsstyleuse112=GetPageRecord('*','greigeRequisition','parentId="'.$lastId.'" and srinkageId="'.$rslistint['materialId'].'" and addFrom="allocation" and greigeAvg!="" and color!="" and salesOrderQty!=""  group by allocationNo order by id asc');
							while($rsstyleuseList12=mysqli_fetch_array($rsstyleuse112)){

							$greall2=GetPageRecord('*','greigeAllocation','allocationNo="'.$rsstyleuseList12['allocationNo'].'" and status=1');
							$greallocat2=mysqli_fetch_array($greall2);
							//if($greallocat2['status']=='1'){
							$requestedQty = $requestedQty + $rsstyleuseList12['requestedQty'];
							//}
							}*/

							//echo $requestedQty;
							//echo $inStockQty.'+++'.$allocationNoData['allocateQty'].'--->';
							//echo ($inStockQty!='' || $allocationNoData['allocateQty']!='') ? $inStockQty+$allocationNoData['allocateQty'] : 0;
							echo $finalAllQty;
							?>
							</td>
							<td><?php
							$rsstyleuse11=GetPageRecord('*','greigeRequisition','parentId="'.$lastId.'" and srinkageId="'.$rslistint['materialId'].'" and addFrom="allocation" and greigeAvg!="" and color!="" and salesOrderQty!="" group by allocationNo order by id asc');
						while($rsstyleuseList=mysqli_fetch_array($rsstyleuse11)){
							$greall=GetPageRecord('*','greigeAllocation','allocationNo="'.$rsstyleuseList['allocationNo'].'" and status=1');
							$greallocat=mysqli_fetch_array($greall);
							?>
							<span style="color: green; font-weight: 700;"><?php echo $greallocat['allocationNo'].' - '.getStyleRefId($greallocat['styleId'])."<br>"; ?></span>
						<?php } ?></td>
							<td><?php
							//echo stripslashes($resListingIns['acceptedField'])-$requestedQty;
							//echo stripslashes($resListingIns['acceptedField'])-$inStockQty;
							?>
							<?php
...
?>
						...
						<?php echo $rslistint['orderQty']-$finalAllQty-$rsExtSum['totalexport']; ?>
						</td>
						</tr>
						<?php } ?>
                            </tr>
