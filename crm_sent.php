				...
				<div class="card">
					...
					<div class="card-body p-0">
						<ul class="nav nav-sidebar" data-nav-type="accordion">
							...
							<li class="nav-item">
								<a href="showpage.crm?module=mail" class="nav-link">
							...
							<li class="nav-item">
								<a href="showpage.crm?module=sent" class="nav-link active">
			...
			<div class="page-header page-header-light">
				<div class="page-header-content header-elements-md-inline">
					<div class="page-title d-flex">
						<h4> <span class="font-weight-semibold">Sent</span> - List</h4>
			...
			<div class="content">
	           ...
	           <div id="loadnewmail"></div>
				...
				<div class="card" id="myinbox">
					<div class="card-header bg-transparent header-elements-inline">
						<h6 class="card-title"><?php echo 'Sent Items';?></h6>
					...
					<!-- Table -->
					<div class="table-responsive" id="shortmail" style="display:none !important;">

					</div>



					<div class="table-responsive" id="composemail">
					...
					$select='*';
					$id='11111';
					$where='queryStatus='.$id.' order by adddate desc';
					$rs=GetPageRecord($select,_QUERYMAILS_MASTER_,$where);
					while($resultpage=mysqli_fetch_array($rs)) {
					?>

							<tr onclick="funsendmailsection('<?php echo $resultpage['id']; ?>');">
									<td class="table-inbox-star rowlink-skip">&nbsp;</td>
									<td class="table-inbox-image">
										<span class="btn bg-indigo-400 rounded-circle btn-icon btn-sm">
											<span class="letter-icon" style="text-transform:uppercase;"><?php echo substr($resultpage['subject'],0,1); ?></span>
										</span>									</td>
									<td class="table-inbox-name">
										<a href="#">
											<div class="letter-icon-title text-default"><?php echo stripslashes($resultpage['fromMail']); ?> - </div>
										</a>									</td>

									<td class="table-inbox-message">
										<span class="table-inbox-subject"><?php echo substr(stripslashes($resultpage['subject']),0,500); ?></span>
									</td>

									<td class="table-inbox-attachment">&nbsp;</td>
									<td class="table-inbox-time"><?php if(date('Y-m-d',strtotime($date))==date('Y-m-d')){ ?> <?php echo date('h:i A',strtotime($date)); ?> <?php } else {  echo date('j M',strtotime($resultpage['adddate'])); } ?></td>
								</tr>
...
function funsendmailsection(id){
$('#composemail').load('load_sent_mail.php?mailid=<?php echo $editresult2['id']; ?>&id='+id);
}
