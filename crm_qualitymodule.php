...
?>
							...
							<td align="center"><?php echo $resultlists['displayId']; ?></td>
								...
								<td><a href="showpage.crm?module=qualitymodule&view=yes&styleid=<?php echo encode($resultlists['id']); ?>"><?php echo '#'.$resultlists['styleRefId']; ?><?php if(countQueryunreadMails($resultlists['id'])!=0){ ?><div class="numberbubbol"><?php echo countQueryunreadMails($resultlists['id']); ?></div><?php } ?></a></td>
		...
		<span style="padding: 5px 10px; background-color: #0097a7; color: #fff; margin-right: 2px; font-size: 12px; margin-bottom:5px;display: inline-block;"><?php echo str_replace(' ','&nbsp;',$lineName['lineName']);?></span>
								...
								<td align="center" class="" style="display:none;">
								 ...
								 <a class="btn btn-primary" style="padding:5px;" href="showpage.crm?module=stylewiselineplan&add=yes&styleid=<?php echo encode($resultlists['id']); ?>&startDate=<?php echo $startDate; ?>&endDate=<?php echo $endDate; ?>&factoryId=<?php echo $factotyData['id']; ?>"><i class="fa fa-eye " aria-hidden="true" style=" color: #fffffff1; font-size: 16px; "></i></a>								  </td>
