...
if($colorId!='' && $qty!='' && $valueEdition!='' && $lining!='' && $datew!=''){

$allvalueAddrs ='colorId="'.$colorId.'",qty="'.$qty.'",valueEdition="'.$valueEdition.'",lining="'.$lining.'",dateAdded="'.$datew.'",styleId="'.decode($_POST['editId']).'"';
...
$namevalue ='name="'.$name.'",startDate="'.$startDate.'",status="'.$status.'",enddate="'.$enddate.'",dateAdded="'.$dateAdded.'",addedBy="'.$_SESSION['userid'].'",buyerId="'.$buyerId.'",brandId="'.$brandId.'"';
...
$namevalue ='name="'.$name.'",status="'.$status.'",startDate="'.$startDate.'",enddate="'.$enddate.'",addedBy="'.$_SESSION['userid'].'",modifyDate="'.$modifyDate.'",modifyBy="'.$_SESSION['userid'].'",buyerId="'.$buyerId.'",brandId="'.$brandId.'"';
...
$namevalue ='name="'.$name.'",line="'.$line.'",description="'.$description.'",dateAdded="'.$dateAdded.'",addedBy="'.$_SESSION['userid'].'"';
...
$namevalue ='name="'.$name.'",line="'.$line.'",description="'.$description.'",modifyDate="'.$modifyDate.'",modifyBy="'.$_SESSION['userid'].'"';
...
$kk ='name="'.$name.'",description="'.$description.'",buyerId="'.$buyerId.'",pdmerchant="'.$pdmerchant.'",productionmerchant="'.$productionmerchant.'",cpm="'.$cpm.'",discount="'.$discount.'"';
...
$namevalue ='name="'.$name.'",description="'.$description.'",dateAdded="'.$dateAdded.'",modifyDate="'.$modifyDate.'",addedBy="'.$_SESSION['userid'].'",modifyBy="'.$_SESSION['userid'].'"';
...
$namevalue ='styleId="'.decode($_POST['styleid']).'",poType="'.$potype.'",poNumber="'.$ponumber.'",requiredDate="'.$rdate.'",poQty="'.$poqty.'",attachFile="'.$file_name.'",factStart="'.$factstart.'",factEnd="'.$factend.'",shipMode="'.$shipmode.'",cutoffDate="'.$cdate.'",deliveryTerm="'.$dterm.'"';
...
$namevalue ='styleId="'.decode($_POST['styleid']).'",colorId="'.decode($_POST['poid']).'",devFtp="'.$file_name1.'",devGtp="'.$file_name2.'",bulkFtp="'.$file_name3.'",ppSample="'.$file_name4.'",bulkGtp="'.$file_name5.'",devFtpDate="'.$date1.'",devGtpDate="'.$date2.'",bulkFtpDate="'.$date3.'",ppSampleDate="'.$date4.'",bulkGtpDate="'.$date5.'",addSampleDate="'.$addsampledate.'",addSample="'.$addsample.'",topSampleDate="'.$topsampledate.'",topSample="'.$topsample.'"';
...
if(trim($_POST['action']) == 'merchantfilehandover'){
...
$kk ='quarter="'.$quarter.'",factoryId="'.$factoryId.'",line="'.$totalLine.'",brandId="'.$brand.'",shiftHours="'.$shiftHrs.'",operator="'.$operator.'",avgSam="'.$avgSam.'",avgEfficiency="'.$avgEfficiency.'",addedBy="'.$_SESSION['userid'].'",outputLine="'.$outputLine.'",outputDay="'.$outputDay.'",outputMonth="'.$outputMonth.'",outputQuarter="'.$outputSeason.'",seasonAllocate="'.$seasonAllocation.'",styleNo="'.$numberOfStyle.'",avgQty="'.$avgQty.'"';
...
$gateentryno=clean($_POST['gateentryno']);
...
$kk ='gateentryno="'.$gateentryno.'",entrydate="'.$entrydate.'",entrytime="'.$entrytime.'",gateno="'.$gateno.'",registerno="'.$registerno.'",potype="'.$potype.'",ponumber="'.$ponumber.'",supplier="'.$supplier.'",billdate="'.$billdate.'",billno="'.$billno.'",vehiclein="'.$vehiclein.'",vehicleout="'.$vehicleout.'",drivername="'.$drivername.'",drivernumber="'.$drivernumber.'",challanno="'.$challanno.'",movement="'.$movement.'",status="2"';

addlisting('gateentrymaster',$kk);
}
else{
...
if($_POST['editId']==""){
$kk ='requisitionno="'.$requisitionno.'",requisitiondate="'.$requisitiondate.'",department="'.$department.'",styleId="'.$styleno.'",indentno="'.$indentno.'",duedate="'.$duedate.'",item="'.$item.'",orderqty="'.$orderqty.'",brandId="'.$brand.'",lot="'.$lot.'",dateAdded="'.$dateAdded.'",requested="'.$requested.'"';



addlisting('requisitionmaster',$kk);
...
$average=trim($_REQUEST['average']);

$pcs=trim($_REQUEST['pcs']);

$fabric=trim($_REQUEST['fabric']);

$where='id="'.$id.'"';

    $kk ='color="'.$color.'",marker="'.$marker.'",size="'.$size.'",average="'.$average.'",pcs="'.$pcs.'",fabric="'.$fabric.'"';


       $update=  updatelisting('requisitiondetailmaster',$kk,$where);
...
$dispatch=trim($_REQUEST['dispatch']);
    ...
    $kk ='qty="'.$qty.'",packages="'.$packages.'",amount="'.$amount.'",dispatch="'.$dispatch.'"';
...
if(trim($_REQUEST['action'])=='issuancedetail'){
...
$where='id="'.$id.'"';
    ...
    $kk ='color="'.$color.'",marker="'.$marker.'",size="'.$size.'",average="'.$average.'",pcs="'.$pcs.'",fabric="'.$fabric.'"';
       ...
       $update=  updatelisting('issuanceDetailMaster',$kk,$where);
...
$requisitionno=clean($_POST['issuanceno']);

$requisitiondate=clean($_POST['issuancedate']);
...
$component=clean($_POST['component']);
...
if($_POST['editId']==""){
$kk ='issuanceno="'.$requisitionno.'",issuancedate="'.$requisitiondate.'",supplierdepartment="'.$department.'",styleId="'.$styleno.'",indentno="'.$indentno.'",duedate="'.$duedate.'",item="'.$item.'",orderqty="'.$orderqty.'",brandId="'.$brand.'",lot="'.$lot.'",component="'.$component.'",dateAdded="'.$dateAdded.'"';
