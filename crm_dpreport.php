                ...
                <form action="" method="get">
                  <div class="col-md-12" style="padding:0px;">
                    ...
                    <select name="stylerefid" id="stylerefid" class="select2" style="margin-left: 10px; width: 190px; padding: .4375rem .875rem; font-size: .8125rem; line-height: 1.5385; color: #333; background-color: #fff; background-clip: padding-box; border: 1px solid #ddd; border-radius: .1875rem; box-shadow: 0 0 0 0 transparent; transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out; height: 2.25003rem; outline: none !important;">
                      ...
                      <?php
        $fcref=GetPageRecord('*','queryMaster','1 and subject!="" and sampleStyle=1 order by id desc');
                  ...
                  <table class="table table-bordered table-hover no-footer">
                    <thead style="background-color: #f5f5f5;">
                     	<tr class="card-body" style="background-color: #f9f9f9;">
                     	    							  <td width="17%" align="left"><div align="center"><strong>Style </strong></div></td>
...
while($resultlists=mysqli_fetch_array($rs[0])){
				...
				$selectversion='*';
				$whereversion='styleId="'.$resultlists['id'].'" order by id desc';
				...
				$rrrr=GetPageRecord('*','operationbulletinentry','1 and styleId="'.$resultlists['id'].'" and costsheetVersionId="'.$costsheetVersionId.'"');
...
?>
                      <tr>
             <td style="text-align:center;"><a  href="showpage.crm?module=dpreport&add=yes&styleid=<?php echo encode($resultlists['id']); ?>"><?php echo '#'.$resultlists['styleRefId']; ?></a></td>
                        <td><?php echo $resultlists['smv']; ?></td>

                        <td><?php echo $operationData['target']; ?></td>
                        <td>
                      	...
                      	$km=GetPageRecord('*','factoryMaster','id="'.$lineData['factoryId'].'"');
								...
								?>
                                                ...
                                                <td align="center"><?php if($lineName['lineName']=='') echo ''; else { ?><span style="padding: 5px 10px; background-color: #0097a7; color: #fff; margin-right: 2px; font-size: 12px; margin-bottom:5px;display: inline-block;"><?php echo str_replace(' ','&nbsp;',$lineName['lineName']);?></span><?php } ?></td>
