...
$select1='*';
$where1='id="'.$id.'"';
$rs1=GetPageRecord($select1,'transferMaster',$where1);
$editresult=mysqli_fetch_array($rs1);
			...
			<div class="content">
				...
				<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
					...
					<div class="page-title d-flex">
						<h4><i class="icon-arrow-left52 mr-1" style="cursor:pointer;"  onclick="cancel();" ></i> Transfer Detail</h4>
...
<input name="editId" type="hidden" id="editId" value="<?php echo encode($lastId); ?>" /><?php
  ...
  <input name="savenew" type="hidden" id="savenew" value="0" />
			...
			<input type="file" name="transferGallary[]" id="transferGallary" onchange="form.submit('#imageglry');" multiple="multiple" style="display:none;" />
			<input type="hidden" name="sectionType" value="transfer"  />
			<input type="hidden" name="parentId" value="<?php echo encode($id); ?>"  />
			<input type="hidden" name="action" value="transferGallary"  />
			</form>
			<script>

			function fundeleteimg(imgId){
				if (confirm("Are you sure?")) {
				 window.location.href='page.de?section=transferMaster&view=yes&id=<?php echo $_GET['id']; ?>&deleteImgId='+imgId;
				}
				return false;
			}

			function funuploadgallary(){
				$('#transferGallary').trigger('click');
