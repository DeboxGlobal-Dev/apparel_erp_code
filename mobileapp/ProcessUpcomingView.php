    ...
    $ids=$_GET['id'];
     ...
     $dateo=Date('Y-m-d', strtotime('+14 days'));
        ...
        $rs14=GetPageRecord('*','timeActionReport','1 and responsiblity="'.$ids.'" and complitionDate>="'.$dateo.'" and actualDate="1970-01-01" and taskListId in (select id from taskListMaster where tna=1)  order by complitionDate asc');
        while($data4=mysqli_fetch_array($rs14)){
