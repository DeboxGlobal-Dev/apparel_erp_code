     ...
     $dateo=Date('Y-m-d', strtotime('+14 days'));
        $rs1=GetPageRecord('responsiblity,COUNT(responsiblity) as rescount','timeActionReport','1 and complitionDate>="'.$dateo.'" and taskListId in (select id from taskListMaster where tna=1) and responsiblity!=""  group by responsiblity order by complitionDate asc');
