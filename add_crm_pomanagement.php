...
<td><div style="text-transform:capitalize;color: white"><b>PO Quantity</b></div></td>
...
<td><div style="text-transform:capitalize;color: white"><b>Ship Mode</b></div></td>
<td><div style="text-transform:capitalize;color: white"><b>Delivery Term</b></div></td>
<td><div style="text-transform:capitalize;color: white"><b>Cutoff Date</b></div></td>
...
<td><div style="text-transform:capitalize;color: white;text-align: center;"><b>Action</b></div></td>
		...
		<tbody>
			<tr>
             ...
             <td><i class="fa fa-pencil"  style="cursor:pointer;" onclick="opmodalpop('PO Management','modalpop.php?action=poManageedit&styleId=<?php echo encode($resultlists['id']); ?>&poid=<?php echo encode($operationData['id']); ?>','600px','auto');" data-toggle="modal" data-target="#modalpop"></i></td>
			  ...
			  <td> <?php if($operationData['cutoffDate'] !="") { echo date('d-m-Y',strtotime($operationData['cutoffDate'])); } else{ echo "-"; } ?> </td>
				...
				<td><div style="text-align:center;"><a href="#" onclick="opmodalpop('PO Breakup','modalpop.php?action=addPoBreakup&id=<?php echo $operationData['id']; ?>&styleId=<?php echo $operationData['styleId']; ?>','600px','auto');" data-toggle="modal" data-target="#modalpop">+Add Break Up</a></div></td>
			</tr>
		    </tbody>
					...
					<tr style="background-color: #fdffe0;">
						...
						<td><i class="fa fa-edit" onclick="opmodalpop('PO Breakup','modalpop.php?action=addPoBreakup&id=<?php echo $rrData['parentId']; ?>&styleId=<?php echo $rrData['styleId']; ?>&editId=<?php echo $rrData['id']; ?>','600px','auto');" data-toggle="modal" data-target="#modalpop" style="font-size: 16px; color: #FF0000; cursor:pointer;"></i></td>
					</tr>
					<?php $total++; }
					if($total==1){
					?>
