                  ...
                  <div class="row">
                    <div class="col-md-4">
                      <div class="form-group">
                        ...
                        <input type="text" name="allocationNo" id="allocationNo" class="form-control readonly" value="<?php echo $allocationNo; ?>"  />
                      </div>
                    </div>
                    <div class="col-md-4">
                      <div class="form-group">
                        <label>Allocation Date</label>
                        <input type="text" name="allocationDate" id="allocationDate" class="form-control " value="<?php if($editresultstyle['allocationDate']!=''){ echo date('d-M-Y', strtotime($editresultstyle['allocationDate'])); }else{ echo date('d-M-Y'); } ?>" readonly/>
                      </div>
                    </div>
                    <div class="col-md-4">
                      <div class="form-group">
                        <label>Style No</label>
                        <select id="greigeStyleNo" name="greigeStyleNo" class=" form-control" displayname="styleNo" onchange="changeIndentNumber(this.value);">
						...
						?>
                          <option value="<?php echo strip($resListing['styleNo']); ?>" <?php if($resListing['styleNo']==$greigeStyleNo){ ?>selected="selected"<?php } ?>><?php echo strip($resListing['styleNo']); ?></option>
                          <?php } ?>
                        </select>
                      </div>
                    </div>
                  ...
                  <div class="row" style="margin-top:10px;">
                    <div class="col-md-4">
                      <div class="form-group">
                        ...
                        <select id="indentNumber" name="indentNumber" class=" form-control" displayname="indentNumber" >
                        </select>
                      </div>
                    </div>
                    <script>
function changeIndentNumber(greStyleNo){
	...
	$('#indentNumber').load('newaction.php?action=changeindentNumbergreige&greStyleNo='+greStyleNo+'&selectId=<?php echo $indentNumber; ?>');
...
if($_GET['id']!=''){
    ...
    changeIndentNumber('<?php echo $greigeStyleNo; ?>');
    	var indentids= $('select#indentNumber').val();

    // alert('<?php echo $indentNumber; ?>');
    </script>
                    <?php
}
?>
                    <div class="col-md-4">
                      <div class="form-group">
                        ...
                        <select id="styleId" name="styleId" class=" form-control" displayname="Season" onchange="loadindentdata(this.value,0,0);">
                          <option value="">Select</option>
                          <?php
						$select='';
						$where='';
						...
						while($resListing=mysqli_fetch_array($rs)){
						?>
                          <option value="<?php echo strip($resListing['id']); ?>" <?php if($resListing['id']==$styleId){ ?>selected="selected"<?php } ?>><?php echo strip($resListing['styleRefId']); ?></option>
                          <?php } ?>
                        </select>
                      </div>
                    </div>
...
function loadindentdata(styleId,allocationRowAddId,did){
  ...
  var greStyleNo = $('#greigeStyleNo').val();
	...
	$('#loadtabledata').load('loadgreigeallocation.php?action=loadindenttabledata&indentid='+indentid+'&styleId='+styleId+'&allocationNo='+allocationNo+'&allocationRowAddId='+allocationRowAddId+'&did='+did+'&greStyleNo=' + greStyleNo);

}
// loadindentdata('<?php echo $styleId; ?>');
$( function(){
	$("#allocationDate" ).datepicker({ minDate: 0 });
} );
...
display:none;
	...
	while($resListing=mysqli_fetch_array($rs)){
	?>
                          <option value="<?php echo $resListing['id']; ?>" <?php  if($brandId==$resListing['id']){ ?>selected="selected" <?php }  ?>><?php echo $resListing['name']; ?></option>
                          <?php } ?>
                        </select>
                      </div>
                    </div>
                    <div class="col-md-4">
                      <div class="form-group">
                        ...
                        <select id="seasonUsedId" name="seasonUsedId" class=" form-control" displayname="Season">
                          <option value="">Select</option>
                          <?php
				...
				while($resListing=mysqli_fetch_array($rs)){
				?>
                          <option value="<?php echo strip($resListing['id']); ?>" <?php if($resListing['id']==$seasonUsedId){ ?>selected="selected"<?php } ?>><?php echo strip($resListing['name']); ?></option>
                          <?php } ?>
                        </select>
                      </div>
                    </div>
            ...
            <div class="card-body">
              <div class="form-group">
				...
				<!--<div style="padding:10px" id="availabeQtyDiv">
					<span style="background: #dbdbdb; padding: 5px;">Available Greige :10000</span>
					<span style="background: #dbdbdb; padding: 5px;">Available Greige2 :10000</span>
				</div>-->
