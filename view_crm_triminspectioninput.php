        ...
        <div class="col-xl-12">
          <div class="card">
            <div class="card-header bg-white">
              <h6 class="card-title">Trim Inspection Input </h6>
            </div>
            <div class="card-body">
              <div class="form-group">
                <div class="row">
                  <table width="100%" border="0" cellpadding="5" cellspacing="0" class="table table-bordered table-responsive input-table" style="font-size:11px !important;">
                    <tr height="60" style="padding: 10px; font-size: 11px; font-weight: 600; background-color: #F8F8F8; margin-top: 10px; position: relative;">
                      ...
                      <td width="81" align="center"><div align="center">P.O No.<?php echo decode($_GET['materialid']); ?></div></td>
				...
				if(id==1){
				$("#addrow").load('loadtrimdata.php?add=1&styleId=<?php echo encode($lastId); ?>&costsheetVersionId=1&lotId=<?php echo decode($_REQUEST['lotId']); ?>&materialid=<?php echo $_GET['materialid']; ?>&colorid=<?php echo $_GET['colorid']; ?>&poid=<?php echo $_GET['poid']; ?>&grnid=<?php echo $_GET['grnid']; ?>');
				}else{
				$("#addrow").load('loadtrimdata.php?styleId=<?php echo encode($lastId); ?>&lotId=<?php echo decode($_REQUEST['lotId']); ?>&materialid=<?php echo $_GET['materialid']; ?>&colorid=<?php echo $_GET['colorid']; ?>&poid=<?php echo $_GET['poid']; ?>&grnid=<?php echo $_GET['grnid']; ?>');
				}
				...
				if(checkyes==true){
				$('#addrow').load('loadtrimdata.php?id='+id+'&deletestatus=yes&styleId=<?php echo encode($lastId); ?>&lotId=<?php echo decode($_REQUEST['lotId']); ?>&materialid=<?php echo $_GET['materialid']; ?>&colorid=<?php echo $_GET['colorid']; ?>&poid=<?php echo $_GET['poid']; ?>&grnid=<?php echo $_GET['grnid']; ?>');
				}
				}
				</script>
                  </table>
                ...
                <div class="row" style="margin-top: 20px;">
                  <?php
$rl=GetPageRecord('*','qualityreportmaster','1 and styleId="'.decode($_REQUEST['styleid']).'" and type="triminspectioninput" and lotId="'.decode($_REQUEST['lotId']).'"');
$trimData=mysqli_fetch_array($rl);


$trimDataq=GetPageRecord('sum(okayqty) as totalokayqty,sum(rejectedqty) as totalrejectedqty,sum(disputedqty) as totaldisputeqty','trimdatamaster','1 and styleId="'.decode($_REQUEST['styleid']).'" and costsheetVersionId=1 and status=1 and deletestatus=0 and lotNoMaster="'.decode($_REQUEST['lotId']).'"');
...
?>
                  <table cellpadding="5" cellspacing="0" style="width:100%;">
                    ...
                    <tr height="30">
                      <td><div align="center">
                          <input name="accepted"  type="text"  id="accepted" value="<?php echo $trimDataqq['totalokayqty'] ?>" autocomplete="off" style="width: 150px; text-align: center; border: 1px solid #bebebe; background-color: #fdfdfd;" onkeyup="savequalityreport();" readonly="readonly">
                        </div></td>
                      <td><div align="center">
                          <input name="reprocessing" type="text"  id="reprocessing" value="<?php echo $trimDataqq['totaldisputeqty'] ?>" autocomplete="off" style="width: 150px; text-align: center; border: 1px solid #bebebe; background-color: #fdfdfd;" onkeyup="savequalityreport();" readonly="readonly">
                        </div></td>
                      <td><div align="center">
                          <input name="rejectedreplaced" type="text"  id="rejectedreplaced" value="<?php echo $trimDataqq['totalrejectedqty'] ?>" autocomplete="off" style="width: 150px; text-align: center; border: 1px solid #bebebe; background-color: #fdfdfd;" onkeyup="savequalityreport();" readonly="readonly">
                        </div></td>
                      <td><div align="center">
                          <input name="onhold" type="text"  id="onhold" value="<?php echo $trimDataqq['totaldisputeqty'] ?>" autocomplete="off" style="width: 150px; text-align: center; border: 1px solid #bebebe; background-color: #fdfdfd;" onkeyup="savequalityreport();" readonly="readonly">
                        </div></td>
