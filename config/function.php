...
{
    mail("rohit.durgapal@deboxglobal.com", "Error Reporting From Debox CRM - " . date('d/m/Y h:i a') . "", $data);
}
//----------------Get Record---------------------
...
function GetPageRecord($select, $tablename, $where)
{
    ...
    $rs = mysqli_query(db(),$sql) or die(senderror(mysqli_error(db())));
...
function login($username, $password)
{
    ...
    $result = mysqli_query(db(),"select * from " . _USER_MASTER_ . " where email='" . $username . "' and  password='" . md5($password) . "' and status=1 ") or die(mysqli_error(db()));
    $number = mysqli_num_rows($result);
    if ($number > 0) {
...
function getMaterialName($id)
{
    if ($id != '') {
        $selectaa = 'name';
        $whereaa = 'id="' . $id . '"';
        $rsaa = GetPageRecord($selectaa, 'materialMaster', $whereaa);
        while ($userss = mysqli_fetch_array($rsaa)) {
            return $userss['name'];
        }
    }
...
function getHsnCode($id)
{
    if ($id != '') {
        $selectaa = 'harcode';
        $whereaa = 'id="' . $id . '"';
        $rsaa = GetPageRecord($selectaa, 'hsncodeMaster', $whereaa);
        while ($userss = mysqli_fetch_array($rsaa)) {
            return $userss['harcode'];
        }
    }
}
...
{
    if ($id != '') {
        $selectaa = 'profileName';
        $whereaa = 'id="' . $id . '"';
        $rsaa = GetPageRecord($selectaa, 'profileMaster', $whereaa);
        while ($userss = mysqli_fetch_array($rsaa)) {
            return $userss['profileName'];
        }
    }
}
...
function getPoTypeName($id)
{
    if ($id != '') {
        $selectaa = 'name';
        $whereaa = 'id="' . $id . '"';
        $rsaa = GetPageRecord($selectaa, 'poTypeMaster', $whereaa);
        $userss = mysqli_fetch_array($rsaa);
        return $userss['name'];
...
function getProcessLossName($id)
{
    if ($id != '') {
        $selectaa = 'name';
        $whereaa = 'id="' . $id . '"';
        $rsaa = GetPageRecord($selectaa, 'processLossMaster', $whereaa);
        $userss = mysqli_fetch_array($rsaa);
        return $userss['name'];
...
function getColorName($id)
{
    if ($id != '') {
        $selectaa = 'name';
        $whereaa = 'id="' . $id . '"';
        $rsaa = GetPageRecord($selectaa, 'colorCardMaster', $whereaa);
        $userss = mysqli_fetch_array($rsaa);
        return $userss['name'];
...
function getSampleTypeName($id)
{
    if ($id != '') {
        $selectaa = 'name';
        $whereaa = 'id="' . $id . '"';
        $rsaa = GetPageRecord($selectaa, 'sampleTypeMaster', $whereaa);
        $userss = mysqli_fetch_array($rsaa);
        $firstFive = explode(' ', $userss['name']);
        return $firstFive[0];
    }
}

function getProductionStageName($id)
{
    if ($id != '') {
        $selectaa = 'name';
        $whereaa = 'id="' . $id . '"';
        $rsaa = GetPageRecord($selectaa, 'productionStageMaster', $whereaa);
        $userss = mysqli_fetch_array($rsaa);
        return $userss['name'];
...
function getActivityTypeName($id)
{
    if ($id!= '') {
        $selectaa = 'name';
        $whereaa = 'id="' . $id . '"';
        $rsaa = GetPageRecord($selectaa, 'activityTypeMaster', $whereaa);
        $userss = mysqli_fetch_array($rsaa);
        return $userss['name'];
...
function GetOrderSequenceNo()
{
	$sequenceNo=1;
	$returnval=0;
	if(file_exists('orderseq.txt'))
	{
		$contents = file_get_contents('orderseq.txt');
	...
	file_put_contents ('orderseq.txt',$contents);

	return $returnval;

}

function getSizeRatio($id)
{
    if ($id!='') {
        $selectaa = 'name';
        $whereaa = 'id="'.$id.'"';
        $rsaa = GetPageRecord($selectaa, 'sizeRatioMaster', $whereaa);
        $userss = mysqli_fetch_array($rsaa);
        return $userss['name'];
...
function getSizeRange($id)
{
    if ($id!='') {
        $selectaa = 'name';
        $whereaa = 'id="'.$id.'"';
        $rsaa = GetPageRecord($selectaa, 'sizerangeMaster', $whereaa);
        $userss = mysqli_fetch_array($rsaa);
        return $userss['name'];
...
function getColumnName($tableName,$id)
{
    if ($id != '') {
        $selectaa = 'name';
        $whereaa = 'id="' . $id . '"';
        $rsaa = GetPageRecord($selectaa, $tableName, $whereaa);
        $userss = mysqli_fetch_array($rsaa);
        return $userss['name'];
...
function getAccountName($id)
{
    if ($id != '') {
        ...
        if($Name=="CP"){
            ...
            $whereaa = 'supplierId="'.$id.'"';
            $tableName = 'suppliersMaster';
        }
        else if($Name=="SP"){
            $whereaa = 'supplierId="'.$id.'"';
            $tableName = 'suppliersMaster';
