...
<div class="page-content">
  ...
  <div class="content-wrapper">
    ...
    <div class="content pt-0" style="margin-top:20px;">
      ...
      <div class="row">
        <div class="col-xl-12">
          <div class="card mb-0 rounded-bottom-0" style="margin-bottom: 10px !important; padding: 10px 5px; width: 100%; text-align: right; float: right; display: block;"> <a style="width: fit-content; float: right;" href="#" onclick="opmodalpop('Email Cost Sheet','modalpop.php?action=emailCostSheet&styleId=<?php echo $_GET['styleid']; ?>','400px','auto');" data-toggle="modal" data-target="#modalpop" class="btn bg-orange-400" aria-expanded="false"><i class="fa fa-envelope" aria-hidden="true" style="margin-right:5px;"></i> Cost Sheet</a> </div>
          <?php
		$i = 0;
		$buyerstatus=0;
		$costsheetVersionId='0';
		$selectversion='*';
		$whereversion='styleId="'.decode($_GET['styleid']).'" and versionId in(select defaultcostsheetVersionId from queryMaster where defaultcostsheetVersionId>0 and id="'.decode($_GET['styleid']).'") or (buyerCostStatus=1 and styleId="'.decode($_GET['styleid']).'") order by id desc';
...
background-color: #d8ff001f !important;
              ...
              <div class="abcspecial-class card-header text-default collapsed" onclick="showfobbydefault<?php echo $costsheetVersionId;?>();" data-toggle="collapse" href="#accordion-item-group<?php echo $resListingVer['id']; ?>" style="cursor:pointer;">
                <h6 class="card-title "> <a onclick="showfobbydefault<?php echo $costsheetVersionId;?>();" data-toggle="collapse" href="#accordion-item-group<?php echo $resListingVer['id']; ?>" class="text-default collapsed" style="color: #000000; width: fit-content; float: left; display: inline-block;margin-bottom: 10px;">
                  <?php if($buyerCostStatus==0){ ?>
                  Internal
                  <?php }else{ ?>
                  Buyer
                  <?php } ?>
				...
				?>
                  <button type="button" class="btn btn-danger" style="float: right; margin: 0px; margin-bottom: 10px;" onclick="duplicateCostsheet<?php echo $costsheetVersionId; ?>();">Create Buyer Cost Sheet <i class="fa fa-copy ml-2" aria-hidden="true" style="margin:0px;"></i></button>
                  <?php } ?>
                  <style>
