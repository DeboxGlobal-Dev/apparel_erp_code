...
<div class="page-content">
 ...
 <a href="#" onclick="opmodalpop(' Add Country','modalpop.php?action=addcountry','600px','auto');"  data-toggle="modal" data-target="#modalpop" class="btn bg-teal-400"  aria-expanded="false"  style="    background-color: #03d873b8;"><i class="fa fa-plus" aria-hidden="true"></i> Create New</a>
					...
					<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer"> <div class="datatable-scroll"><table class="table table-bordered table-hover datatable-highlight dataTable no-footer" id="DataTables_Table_2" role="grid" aria-describedby="DataTables_Table_2_info">
						<thead>
							<tr role="row">
							  ...
							  <th width="20%" rowspan="1" align="left" class="sorting" tabindex="0" aria-controls="DataTables_Table_2" aria-label="Last Name: activate to sort column ascending">Last&nbsp;Modify By </th>
...
$rs=GetRecordList($select,'countryMaster',$where,$limit,$page,$targetpage);
$totalentry=$rs[1];
$paging=$rs[2];
...
?>
						...
						<tr role="row" class="odd">
								<td width="60%" align="left" class="sorting_1"><a   title="<?php echo $resultlists['name']; ?>" style=" color:#2196f3; display:block; min-width:120px; " href="#"  onclick="opmodalpop(' Edit Country','modalpop.php?action=addcountry&id=<?php echo encode($resultlists['id']); ?>','600px','auto');"  class="shorttextcard" data-toggle="modal" data-target="#modalpop"  ><?php echo strip($resultlists['name']); ?></a> </td>
						  <td width="20%" align="left" class=""><div class="shorttextcard" ><?php echo getUserName($resultlists['modifyBy']); ?></div></td>
								<td width="20%" align="left" class=""><?php if($resultlists['modifyDate']!=""){ echo $resultlists['modifyDate']; } ?></td>
