...
$selectimg='name';
$whereimg='id="'.decode($_REQUEST['delid']).'" ';
$rsimg=GetPageRecord($selectimg,'newsAndUpdate',$whereimg);
$imgresult=mysqli_fetch_array($rsimg);

unlink('images/imageGallary/'.$imgresult['name']);

$where='id="'.decode($_REQUEST['delid']).'"';
deleteRecord('newsAndUpdate',$where);
}
?>
...
<div class="page-content">
 ...
 <a href="#" onclick="opmodalpop(' Add Banner','modalpop.php?action=addbanner','600px','auto');"  data-toggle="modal" data-target="#modalpop" class="btn bg-teal-400"  aria-expanded="false"  style="    background-color: #03d873b8;"><i class="fa fa-plus" aria-hidden="true"></i> Create New</a>
					...
					<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer"> <div class="datatable-scroll">
					  <table width="100%" class="table table-bordered table-hover datatable-highlight dataTable no-footer" id="DataTables_Table_2" role="grid" aria-describedby="DataTables_Table_2_info">
                        <thead>
                          <tr role="row">
                            <th rowspan="1" align="left" class="sorting_asc" tabindex="0" aria-controls="DataTables_Table_2" aria-sort="ascending" aria-label="First Name: activate to sort column descending">&nbsp;</th>
                            <th width="4%" rowspan="1" align="left" class="sorting" tabindex="0" aria-controls="DataTables_Table_2" aria-label="Last Name: activate to sort column ascending">Date</th>
                          </tr>
                        </thead>
                        <tbody>
                          <?php
...
$where='where  '.$wheresearchassign.' '.$wheresearch.' and imgUrl!="" '.$assignto.' order by id desc';
$page=$_GET['page'];

$targetpage=$fullurl.'showpage.crm?module="'.$modfile['moduleName'].'"&records='.$limit.'&searchField='.$searchField.'&assignto='.$_GET['assignto'].'&';
$rs=GetRecordList($select,'banner',$where,$limit,$page,$targetpage);
$totalentry=$rs[1];
$paging=$rs[2];
...
?>
                          <tr role="row" class="odd">
                            <td width="5%" align="left" class="sorting_1"><img src="images/imageGallary/<?php echo $resultlists['imgUrl']; ?>" alt="<?php echo $resultlists['imgUrl']; ?>" height="60" class="card-img img-fluid" style=" height:60px !important; width:auto !important;"></td>
                            <td width="4%" align="left" class=""><?php echo date('d/m/Y',$resultlists['updatedDate']); ?></td>
