  ...
  $(function () {
    ...
    $('#calendar').fullCalendar({
	  ...
	  events    : [
		...
		$rs=GetPageRecord($select,'bookembroideryMaster',$where);
		while($calldata=mysqli_fetch_array($rs)){

		?>
	  	{
          title          : '<?php echo '#'.getStyleRefId($calldata['styleid']).' - Limit: '.$calldata['approveLimit']; ?>',
          ...
          allDay         : false,
		   ...
		   url            : 'clicknew(<?php echo $calldata['id']; ?>);',
...
$(document).on('click',function(e) {
 ...
 if (typeof $this.attr('data-date') === "undefined") {} else {
    var mid= $this.attr('data-date');
     opmodalpop('Book Embroidery','modalpop.php?action=bookembroideryMaster&adddate='+mid+'','600px','auto');
	 }
});

function clicknew(id){
opmodalpop('Book Embroidery','modalpop.php?action=bookembroideryMaster&id='+id+'','600px','auto');
}
...
<div class="content-wrapper">
  ...
  <a href="showpage.crm?module=<?php echo $_GET['module']; ?>&view=yes" class="btn bg-teal-400 addnotify" aria-expanded="false" style="    background-color: #03d873b8;"  ><i class="fa fa-eye" aria-hidden="true"></i> List View</a>
