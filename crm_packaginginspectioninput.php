...
?>
						...
						$grnDataq=GetPageRecord('*','grnMaster','1 and styleId="'.$resultlists['id'].'"  and parentId in (select id from grnMaster where grnNo!="") and materialId in (select id from styleSubCategoryMaster where materialType=3) order by id');
            ...
            $rsgrnMee=GetPageRecord('id','materialMaster','name="'.$materialData['name'].'"');
            $rsgrnMNameee=mysqli_fetch_array($rsgrnMee);

$trimDatacountq=GetPageRecord('id','packagaingtrimdatamaster','1 and styleId="'.$resultlists['id'].'" and costsheetVersionId=1 and status=1 and deletestatus=0 and lotNoMaster="'.$lotNo.'" order by id asc');
$trimDatacount=mysql_num_rows($trimDatacountq);
    		 ?>
						  ...
						  <td><?php echo $materialData['name']; ?></td>
						   <td><?php
						$rs112=GetPageRecord('name,id','colorCardMaster','id="'.$grnData['color'].'"');
						$resListing112=mysqli_fetch_array($rs112);
						echo $resListing112['name'];
						?>
					       </td>
					         <?php
                $rsgrnrec=GetPageRecord('sum(netReceived) as netReceivedTill,color,parentId','grnMaster','styleId="'.$resultlists['id'].'" and materialId="'.$materialData['id'].'" and color="'.$resListing112['id'].'"');
				$rsgrnrecTill=mysqli_fetch_array($rsgrnrec);
                                    ?>
                                    <td><?php echo round($rsgrnrecTill['netReceivedTill'],2); ?></td>
						   <td><a href="showpage.crm?module=<?php echo $_GET['module']; ?>&view=yes&styleid=<?php echo encode($resultlists['id']); ?>&lotId=<?php echo encode($lotNo); ?>&grnid=<?php echo encode($grnData['parentId']); ?>&materialid=<?php echo encode($grnData['materialId']); ?>&colorid=<?php echo encode($grnData['color']); ?>&grnids=<?php echo encode($grnData['id']); ?>&poid=<?php echo encode($grnData['supplierPurchaseOrderId']); ?>&msid=<?php echo $rsgrnMNameee['id']; ?>" style="padding: 5px 10px; border: 1px solid #0097a7; color: #0288d1; font-size: 12px; margin-bottom: 0px; margin-right: 5px; display: inline-block;">Lot <?php echo $lotNo; ?></a></td>
						  <td><?php if($trimDatacount>0){ ?> <span style="color: #368006;"><strong>Completed</strong></span><?php } else{ ?> <span style="color: #ff0000;"><strong>Pending</strong></span><?php } ?> </td>
						  </tr>
						  <?php } } else{ ?>
						  <tr>
