...
?>
						...
						$grnDataq=GetPageRecord('*','grnMaster','1 and styleId="'.$resultlists['id'].'"  and parentId in (select id from grnMaster where grnNo!="") and materialId in (select id from styleSubCategoryMaster where materialType=2) order by id');
...
$trimDatacount=mysql_num_rows($trimDatacountq);
    		 ?>
				...
				$rsgrnrecTill=mysqli_fetch_array($rsgrnrec);
						  ...
						  <td><a href="showpage.crm?module=<?php echo $_GET['module']; ?>&view=yes&styleid=<?php echo encode($resultlists['id']); ?>&lotId=<?php echo encode($lotNo); ?>&grnid=<?php echo encode($grnData['parentId']); ?>&materialid=<?php echo encode($grnData['materialId']); ?>&colorid=<?php echo encode($grnData['color']); ?>&poid=<?php echo encode($grnData['supplierPurchaseOrderId']); ?>&msid=<?php echo $rsgrnMNameee['id']; ?>" style="padding: 5px 10px; border: 1px solid #0097a7; color: #0288d1; font-size: 12px; margin-bottom: 0px; margin-right: 5px; display: inline-block;">Lot <?php echo $lotNo; ?></a></td>
