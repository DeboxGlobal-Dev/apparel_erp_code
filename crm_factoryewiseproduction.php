  ...
  padding:5px;
   ...
   <div class="f-div" style="width:60%; float:left; overflow-y:auto;">
     ...
     <?php
              ...
              <table cellspacing="0" cellpadding="5" class="table-bordered">
                <tr style="background-color: #0288d1; font-weight: 500; text-align: center; color: #fff; font-size: 15px;">
                  <?php
					 $factoryNameFirstq=GetPageRecord('*','factoryMaster','1 and status=1 and deletestatus=0 order by name asc');
					while($factoryNameFirst=mysqli_fetch_array($factoryNameFirstq)){  ?>
					<td><div><?php echo $factoryNameFirst['name']; ?></div></td>
					 <?php } ?>
                </tr>
				<tr>
                  <?php
					 ...
					 //================================================
					$kr=GetPageRecord('*','factoryMaster','1 and status=1 and deletestatus=0 order by name asc');

					while($factoryData=mysqli_fetch_array($kr)){
...
$aa=GetPageRecord('sum(dateWiseLineInput) as totaldateWiseLineInput','linePlanMaster','1 and factoryId="'.$factoryData['id'].'" and uploadInputDate="'.$abc.'"');
$getstyleid=mysqli_fetch_array($aa);

$km=GetPageRecord('SUM(loading) as totalLoading,SUM(output) as totalOutput','recorderInputMaster','1 and factoryId="'.$factoryData['id'].'" and fromDate="'.$abc.'"');
...
$aa=GetPageRecord('sum(dateWiseLineInput) as totaldateWiseLineInput','linePlanMaster','1 and uploadInputDate="'.$abc.'"');
$getstyleid=mysqli_fetch_array($aa);

$km=GetPageRecord('SUM(loading) as totalLoading,SUM(output) as totalOutput','recorderInputMaster','1 and fromDate="'.$abc.'"');
$recorderInputData=mysqli_fetch_array($km);
