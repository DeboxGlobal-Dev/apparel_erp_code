...
background-color: #0097a71a;
  ...
  <a href="#" onclick="opmodalpop(' Add Vendor','modalpop.php?action=<?php echo $_GET['module']; ?>','600px','auto');" data-toggle="modal" data-target="#modalpop" class="btn bg-teal-400" aria-expanded="false" style="background-color: #03d873b8;"><i class="fa fa-plus" aria-hidden="true"></i> Create New</a>
				...
				<div class="card">
					...
					<table class="table table-bordered table-hover datatable-highlight dataTable no-footer" id="DataTables_Table_2" role="grid" aria-describedby="DataTables_Table_2_info">
						<thead style="background-color: #f5f5f5;">
							<tr role="row">
								<th class="sorting_asc" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-sort="ascending">Vendor ID</th>
								<th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1">Vendor&nbsp;Name</th>
...
$rs=GetRecordList($select,_VENDOR_MASTER_,$where,$limit,$page,$targetpage);
$totalentry=$rs[1];
$paging=$rs[2];
...
?>
							<tr role="row" class="odd">
								...
								<div class=""><a href="showpage.crm?module=vendors&view=yes&id=<?php echo encode($resultlists['id']); ?>">
								<?php echo $resultlists['supplierId']; ?>
								</a>								</div>								</td>

								<td><?php echo makeQueryId($resultlists['name']); ?></td>

								<td><?php echo $resultlists['email']; ?></td>
								<td><?php echo $resultlists['phone']; ?></td>
							    <td><?php
								$a=GetPageRecord('*','queryCurrencyMaster','1 and id="'.$resultlists['supplierCurrency'].'"');
								$currenname=mysqli_fetch_array($a);
								echo $currenname['name'];
								?></td>
						       <td align="center"><a href="showpage.crm?module=vendors&view=yes&id=<?php echo encode($resultlists['id']); ?>" class="btn btn-primary" style="padding:5px;"><i class="fa fa-eye " aria-hidden="true" style=" color: #fffffff1; font-size: 16px; "></i></a></td>
							</tr>
