...
$grnid=decode($_REQUEST['grnid']);
                        ...
                        $materialDataq=GetPageRecord('*','styleSubCategoryMaster','id="'.$_REQUEST['mat'].'"');
...
$rsr=GetPageRecord($select,'packagaingtrimdatamaster',$where);
$resListing1r=mysqli_fetch_array($rsr);

if($_REQUEST['add']==1){
$namevalueadd = 'styleId="'.decode($_REQUEST['styleId']).'",costsheetVersionId=1,addedBy="'.$_SESSION['userid'].'",dateAdded="'.time().'",status=1,lotNoMaster="'.$lotId.'",item_trims="'.$resListing1r['item_trims'].'",item_code="'.$resListing1r['item_code'].'",vendor_name="'.$resListing1r['vendor_name'].'",pono="'.$resListing1r['pono'].'",receivedqty="'.$resListing1r['receivedqty'].'",totalorderqty="'.$resListing1r['totalorderqty'].'",lotno="'.$resListing1r['lotno'].'",lotreceiveddate="'.$resListing1r['lotreceiveddate'].'",recievedutytlot="'.$resListing1r['recievedutytlot'].'",recievedutytillnow="'.$resListing1r['recievedutytillnow'].'",balancetoreceive="'.$resListing1r['balancetoreceive'].'",inspectiondate="'.$resListing1r['inspectiondate'].'",remarks="'.$resListing1r['remarks'].'"';
addlistinggetlastid('packagaingtrimdatamaster',$namevalueadd);
}

if($_REQUEST['deletestatus']=="yes" && $_REQUEST['id']!=''){
deleteRecord('packagaingtrimdatamaster','id="'.$_REQUEST['id'].'"');
}
...
$select='*';
$where=' styleId="'.decode($_REQUEST['styleId']).'" and costsheetVersionId=1 and status=1 and deletestatus=0 and lotNoMaster="'.$lotId.'" order by id asc';
$rs=GetPageRecord($select,'packagaingtrimdatamaster',$where);
...
<td height="20" align="right"><div align="center">
  <input name="item_trims" type="text"  id="item_trims<?php echo $resListing1['id']; ?>" value="<?php echo $mat; ?>" autocomplete="off"  style="width:95px; text-align:center;" onkeyup="savemeasurmentdata<?php echo $resListing1['id']; ?>();" readonly="readonly">
</div></td>
<?php
...
<?php
	...
	$tsq=GetPageRecord('bomPlacement,storesupplier,supplierartname,bomAvg','techPackDetailMaster','1 and stylesubtabid='.$materialData['id'].' order by id asc');                $techShellData=mysqli_fetch_array($tsq);

	?>
<td height="20" align="right"><div align="center">
  <input name="vendor_name" type="text"  id="vendor_name<?php echo $resListing1['id']; ?>" value="<?php echo getsupplierCompany($techShellData['storesupplier']); ?>" autocomplete="off"  style="width:95px; text-align:center;" onkeyup="savemeasurmentdata<?php echo $resListing1['id']; ?>();" readonly="readonly">
</div></td>

<?php
$materialDataqs=GetPageRecord('*','grnMaster','id="'.$_REQUEST['grnids'].'"');
						$materialDatas=mysqli_fetch_array($materialDataqs);

?>
<td height="20" align="right"><div align="center">
  <input name="pono" type="text"  id="pono<?php echo $resListing1['id']; ?>" value="<?php echo $materialDatas['supplierPurchaseOrderId']; ?>" autocomplete="off"  style="width:95px; text-align:center;" onkeyup="savemeasurmentdata<?php echo $resListing1['id']; ?>();" readonly="readonly">
</div></td>


<?php
$materialDataqsa=GetPageRecord('*','indentCreationMaster','id="'.$_REQUEST['required'].'"');
						$materialDatasa=mysqli_fetch_array($materialDataqsa);

?>
<td height="20" align="right"><div align="center">
  <input name="receivedqty" type="text"  id="receivedqty<?php echo $resListing1['id']; ?>" value="<?php echo $materialDatasa['poQty']*$materialDatasa['avg']; ?>" autocomplete="off"  style="width:95px; text-align:center;" onkeyup="savemeasurmentdata<?php echo $resListing1['id']; ?>();" readonly="readonly">
</div></td>
<?php

$total=GetPageRecord('*','indentCreationMaster','id="'.$_REQUEST['total'].'"');
						$totals=mysqli_fetch_array($total);
?>
<td height="20" align="right"><div align="center">
  <input name="totalorderqty" type="text"  id="totalorderqty<?php echo $resListing1['id']; ?>" value="<?php echo $_REQUEST['total']; ?>" autocomplete="off"  style="width:95px; text-align:center;" onkeyup="savemeasurmentdata<?php echo $resListing1['id']; ?>();" readonly="readonly">
</div></td>
<?php
...
<td height="20" align="right"><div align="center">
  ...
  <!--<input name="inspectiondate" type="date"  id="inspectiondate<?php echo $resListing1['id']; ?>" value="<?php if($resListing1['inspectiondate']!='0000-00-00'){ echo date('d-m-Y',strtotime($resListing1['inspectiondate'])); } ?>" autocomplete="off"  style="width:95px; text-align:center;" onchange="savemeasurmentdata<?php echo $resListing1['id']; ?>();" >-->
