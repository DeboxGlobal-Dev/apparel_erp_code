                ...
                <form action="" method="get">
                  <div class="col-md-12" style="padding:0px;">
                    ...
                    <select name="stylerefid" id="stylerefid" class="select2" style="margin-left: 10px; width: 190px; padding: .4375rem .875rem; font-size: .8125rem; line-height: 1.5385; color: #333; background-color: #fff; background-clip: padding-box; border: 1px solid #ddd; border-radius: .1875rem; box-shadow: 0 0 0 0 transparent; transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out; height: 2.25003rem; outline: none !important;">
                      ...
                      <?php
				$fcref=GetPageRecord('*','queryMaster',''.$wheresearchassign.' deleteStatus=0 and id in(select styleId from techPackDetailMaster) and subject!="" order by id desc');
                ...
                <div class="datatable-scroll">
                  <table class="table table-bordered table-hover no-footer" style="width:100%;">
                    <thead style="background-color: #f5f5f5;">
                      <tr role="row">
                        <th width="112"><span class="sorting">Style#</span></th>
                        <th width="339">Style&nbsp;Name</th>
                        <th width="266">Uploaded&nbsp;By</th>
                        <th width="143">Status</th>
                        <th><div align="center">Download</div></th>
                        <?php if($loginuserprofileId==1 || $loginuserprofileId==93 || $loginuserprofileId==92 || $loginuserprofileId==85){ ?>
                        <th width="246"><div align="left"><span class="text-center" style="width: 100px;">Upload&nbsp;Costsheet</span></div></th>
                        <?php } ?>
                       <?php if($loginuserprofileId==1 || $loginuserprofileId==93 || $loginuserprofileId==92 || $loginuserprofileId==85){ ?>
                        <?php } ?>
						 <?php if($loginuserprofileId==1 || $loginuserprofileId==92){ ?>
                        <?php } ?>
                      </tr>
                    </thead>
...
$pdhead=GetPageRecord($selectstatus,'styleAssignmentMaster','styleId="'.$resultlists['id'].'" and statusId=18 order by id desc');
$resultpdhead=mysqli_fetch_array($pdhead);
...
?>
                        ...
                        <td><a href="showpage.crm?module=costsheet&add=yes&styleid=<?php echo encode($resultlists['id']); ?>">
                            <?php echo '#'.$resultlists['styleRefId']; ?> </a></td>


                        <td><?php echo $resultlists['subject']; ?></td>
                        <td><?php echo getUserName($resultlists['costsheetassignTo']); ?>
                          <?php if($resultlists['costsheetdateAdded']!='0'){ echo ' - '.date('d M, Y - h:i A',$resultlists['costsheetdateAdded']); }else{ echo '-'; }; ?></td>
						...
						<a href="download-costsheet.php?styleId=<?php echo encode($resultlists['id']); ?>&finalversion=<?php echo encode($resultlists['defaultcostsheetVersionId']); ?>" target="_blank" style="background: #0288d1; outline: none; color: #fff; padding: 5px; border-radius: 2px; cursor: pointer; width: 95px; display: block; float:left;"><i class="fa fa-download" aria-hidden="true"></i> Cost Sheet</a>
						<a href="<?php echo $fullurl; ?>tcpdf/examples/generatecostsheet.php?pageurl=<?php echo $fullurl; ?>printcostsheet.php?id=<?php echo encode($resultlists['id']); ?>" target="_blank" style="background: #e71c22; outline: none; color: #fff; padding: 5px; border-radius: 2px; cursor: pointer; width: 95px; display: block; float:right; margin-left:5px;"><i class="fa fa-print" aria-hidden="true"></i> Cost Sheet</a>						</div>						</td>
                        <td align="center"><div align="left"><a href="#" onclick="opmodalpop(' Upload Costsheet','modalpop.php?action=uploadcostsheet&id=<?php echo encode($resultlists['id']); ?>','400px','auto');" data-toggle="modal" data-target="#modalpop">
                          <button style="font-size: 12px; padding: 3px 24px; margin: 0px !important;">Upload <i class="fa fa-upload"></i></button>
                          </a>
                            <?php if($resultlists['uploadedCostsheet']!=''){?>
                        </div>
                          <div style="display: block; margin-top: 10px; font-size: 13px;">
                            <div align="left"><a href="images/<?php echo $resultlists['uploadedCostsheet']; ?>" target="blank"><i class="fa fa-download" aria-hidden="true" style="margin-right: 5px;"></i>View Costsheet</a></div>
                          </div>
                          <div align="left">
                            <?php } ?>
                          </div></td>
