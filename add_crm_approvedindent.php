...
<div class="page-content">
        ...
        <div class="content pt-0" style="margin-top:20px;">
            ...
            <div class="row">
                        ...
                        <div class="panel panel-flat">
                            <div class="table-responsive">
                                    ...
                                    <table width="100%" class="table table-bordered table-responsive forbom"
                                        ...
                                        <tbody style="width: 100%;display: inline-table;">
                                            <tr class="card-body" style="background-color: #fff7b3;;">
                                                ...
                                                <td width="15%" align="center"><strong>Material/Item&nbsp;Id</strong>
                                                </td>
							  ...
							  <td width="12%" align="center"><strong>Total&nbsp;allowance(%)</strong></td>-->
                                                <td width="12%" align="center"><strong>Ordered&nbsp;Qty</strong></td>
							...
							?>
                                            ...
                                            <?php
							 $rs1=GetPageRecord('*','styleSubCategoryMaster','styleId="'.decode($_GET['styleid']).'" and costsheetVersionId="'.$editresultstyle['defaultcostsheetVersionId'].'" and materialType="'.$resListingtype['id'].'" and approved=2 order by sr asc');
							 /// and approved=2 and parentId=0 and sr<100 order by sr asc////
				...
				// 			$totalMaterialValue = round($totalMaterialQty*$resListing12['bomRate'],3);
                            ...
                            $rsssqty='';
                            $rsssqtylist='';
							$rsssqty=GetPageRecord('*','purchaseOrderStyleMaster','1 and parentId="'.$result1['id'].'" and color="'.$color.'"');
						while($rsssqtylist=mysqli_fetch_array($rsssqty)){


							$rsssqtyzx=GetPageRecord('SUM(gdQty) as trsum','purchaseOrderStyleMaster','1 and parentId="'.$result1['id'].'" and color="'.$color.'"');
	$rsssqtylistzx=mysqli_fetch_array($rsssqtyzx);
						...
						$tillTotalMaterialQty=0;
						$rsindent=GetPageRecord('SUM(orderQty) as tillorderQty','indentCreationMaster','styleId="'.$resListing1['styleId'].'" and techpackdetailId="'.$resListing12['id'].'" and styleSubCatTableId="'.$resListing1['id'].'" and color="'.$color.'" and size="'.$size.'" and isCancel="no"');
						...
						if($resListing1['addMaterialFrom']=='greige'){
							$isGrigeClass = '';
							$rsgreige=GetPageRecord('*','greigeRequisition','1 and srinkageId="'.$resListing1['materialid'].'" and color="'.$color.'" and parentId in ( select id from greigeRequisition where styleNo in (select greigeStyleNo from greigeAllocation where styleId="'.decode($_GET['styleid']).'")) order by id asc');
						...
						if($resListing1['addMaterialFrom']=='yarn'){
							$isGrigeClass = '';
							$rsgreige=GetPageRecord('*','yarnRequisition','1 and srinkageId="'.$resListing1['materialid'].'" and color="'.$color.'" and parentId in ( select id from yarnRequisition where styleNo in (select greigeStyleNo from yarnAllocation where styleId="'.decode($_GET['styleid']).'")) order by id asc');
							$resultgreige=mysqli_fetch_array($rsgreige);
								if($resultgreige['color']==$color){
									$isGrigeClass = 'inGreige';
								}else{
						...
						?>
                                            <tr
                                                class="card-body <?php if($resListingtype['name']=="Fabric"){ echo $isGrigeClass; } ?>">
                              ...
                              <td width="12%" align="center"><?php  echo getSupplierName($resListing12['storesupplier']); ?></td>-->
                                                ...
                                                <!--  <td width="12%" align="center"><?php   echo round($resListing12['bomvalueonepc'],3); ?></td>-->
                                                <td width="12%" align="center">
                                                    <?php
							 ...
							 // $perqty = $resultpro['qty']*$perqty/100;
							  ?>
								...
								if($resListing1['allocationNo']==""){
									echo $resListingIndent1['tillorderQty'];
								}else{
									$allocation=GetPageRecord('*','greigeRequisition','srinkageId="'.$resListing1['materialid'].'" and allocationNo="'.$resListing1['allocationNo'].'" order by id desc');
									$allocationData=mysqli_fetch_array($allocation);
									echo round($totalMaterialQty,3);
								}
								?></td>
                                                <td width="12%" align="center">
                                                    <?php if($resListing1['allocationNo']==""){ echo $tillTotalMaterialQty; }else{ echo $tillTotalMaterialQty-$totalMaterialQty; } ?>
                                                </td>
                                                <td width="12%" align="center">
							...
							$rsindent2=GetPageRecord('SUM(orderQty) as tillorderQtysize','indentCreationMaster','styleId="'.$resListing1['styleId'].'" and techpackdetailId="'.$resListing12['id'].'" and styleSubCatTableId="'.$resListing1['id'].'" and color="'.$color.'"  and size="'.$newsize.'"  and isCancel="no"');
