...
$where='where  subject!="" '.$stylestatus.' and sampleStyle="2" and deletestatus=0 order by id desc';
$page=$_GET['page'];
...
?>
							...
							<td align="center"><?php echo $resultlists['displayId']; ?></td>
								<td><a href="showpage.crm?module=samplingindent&add=yes&styleid=<?php echo encode($resultlists['id']); ?>"><?php echo '#'.$resultlists['styleRefId']; ?><?php if(countQueryunreadMails($resultlists['id'])!=0){ ?><div class="numberbubbol"><?php echo countQueryunreadMails($resultlists['id']); ?></div><?php } ?></a></td>
							   ...
							   <td><?php echo $resultqty['qtyTotal']; ?></td>
								...
								<td><a href="showpage.crm?module=indentmpl&add=yes&styleid=<?php echo encode($resultlists['id']); ?>"><span class="badge" style="cursor:pointer;background-color:green; color:#fff; position: relative;width: 142px; font-size: 11px; padding: 6px;">View Indent</span></a></td>
