        ...
        <div class="col-xl-12">
          <div class="card-header header-elements-inline bg-blue-700 filterable" style="padding: 10px;">
			...
			<div class="col-xl-3" style="    padding-right: 0px;"><div class="btn-group justify-content-center" style="float:right;">
							 ...
							 <a href="#" onclick="opmodalpop(' Add TNA Stage','modalpop.php?action=<?php echo $_GET['module']; ?>&stylerefid=<?php echo $_GET['stylerefid']; ?>','600px','auto');" data-toggle="modal" data-target="#modalpop" class="btn bg-teal-400" aria-expanded="false" style="background-color: #03d873b8;"><i class="fa fa-plus" aria-hidden="true"></i> Create New</a>
                ...
                <form action="" method="get">
                  <div class="col-md-12" style="padding:0px;">
                    ...
                    <select name="stylerefid" id="stylerefid" class="select2" style="margin-left: 10px; width: 190px; padding: .4375rem .875rem; font-size: .8125rem; line-height: 1.5385; color: #333; background-color: #fff; background-clip: padding-box; border: 1px solid #ddd; border-radius: .1875rem; box-shadow: 0 0 0 0 transparent; transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out; height: 2.25003rem; outline: none !important;">
                      ...
                      <?php
				      $fcref=GetPageRecord('id,name','tnaTemplatesMaster','1 and name!="" order by id asc');
				       while($refData=mysqli_fetch_array($fcref)){ ?>
                      <option value="<?php echo encode($refData['id']); ?>" <?php if(decode($_GET['stylerefid'])==$refData['id']){ ?> selected="selected" <?php } ?>><?php echo $refData['name']; ?></option>
                      <?php } ?>
                    </select>
                    </label>
...
$where='where  name!="" '.$temQuery.' order by name';
...
$rs=GetRecordList($select,'taskListMaster',$where,$limit,$page,$targetpage);
$totalentry=$rs[1];
$paging=$rs[2];
...
<a href="#" onclick="opmodalpop(' Edit Task','modalpop.php?action=<?php echo $_GET['module']; ?>&id=<?php echo encode($resultlists['id']); ?>&temid=<?php echo encode($resultlists['tnatemplate']); ?>&stylerefid=<?php echo $_GET['stylerefid']; ?>','600px','auto');" data-toggle="modal" data-target="#modalpop">

<?php
$activityquery=GetPageRecord('name','tnaActivityMaster','1 and id="'.$resultlists['name'].'"');
...
<td><input type="number" name="srNooo" id="srNooo" value="<?php echo $resultlists['sr']; ?>" onkeyup="saveSerialNo<?php echo $resultlists['id']; ?>(this.value);" style="width: 50px; text-align: center;" /></td>


<td id="savebomSerial<?php echo $resultlists['id']; ?>" style="display:none;"></td>

<script>

function saveSerialNo<?php echo $resultlists['id']; ?>(id){
$('#savebomSerial<?php echo $resultlists['id']; ?>').load('apparelbomaction.php?action=savetnaSerialNo&id=<?php echo encode($resultlists['id']); ?>&srNooo='+id);
								...
								<td>
								<?php
							    $a=GetPageRecord('name','tnaTemplatesMaster','1 and id="'.$resultlists['tnatemplate'].'" order by name asc');
								$userssaa=mysqli_fetch_array($a);
								echo $userssaa['name'];
							    ?>								</td>


								<td><?php echo $resultlists['totaldays']; ?></td>
								<td>

								<?php
								$rs22=GetPageRecord('name','tnaActivityMaster','1 and id="'.$resultlists['criticalPath'].'"');
								...
								<td><?php if($resultlists['tna']==1){ ?><span class="badge badge-primary">Yes</span><?php } ?><?php if($resultlists['tna']==2){ ?><span class="badge badge-danger">No</span><?php } ?></td>
