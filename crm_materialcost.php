...
$where='where id in (select styleId from styleSubCategoryMaster  where FIND_IN_SET('.$_SESSION['userid'].',assignTo)) and subject!="" and sampleStyle="1" and deletestatus=0 order by id desc';
}elseif($loginuserprofileId==155){

$where='where id in (select styleId from styleSubCategoryMaster  where  FIND_IN_SET('.$_SESSION['userid'].',assignToPurMerchant)) and sampleStyle="1" and subject!="" and deletestatus=0 order by id desc';
}else{
...
?>
					  ...
					  <tr role="row" class="odd">
                        <td><a href="showpage.crm?module=materialcost&add=yes&styleid=<?php echo encode($resultlists['id']); ?>"><?php echo '#'.$resultlists['styleRefId']; ?>
